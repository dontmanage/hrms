...
def create_kras():
	...
	dontmanage.reload_doc("hr", "doctype", "kra")

	template_goals = dontmanage.get_all("Appraisal Template Goal", fields=["name", "kra"], as_list=True)
	...
	for name, kra in template_goals:
		...
		if not dontmanage.db.exists("KRA", kra_title):
			dontmanage.get_doc(
				{
					"doctype": "KRA",
					"title": kra_title,
		...
		dontmanage.db.set_value("Appraisal Template Goal", name, "kra", kra_title, update_modified=False)
...
def update_kra_evaluation_method():
	...
	(
		dontmanage.qb.update(Appraisal)
