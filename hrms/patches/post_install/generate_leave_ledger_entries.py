...
import dontmanage
from dontmanage.utils import getdate, today
...
def execute():
	...
	dontmanage.reload_doc("HR", "doctype", "Leave Encashment")
	dontmanage.reload_doc("HR", "doctype", "Leave Type")
	...
	# ref: https://github.com/dontmanage/dontmanageerp/pull/17624
	if dontmanage.db.a_row_exists("Leave Ledger Entry"):
		return

	if not dontmanage.get_meta("Leave Allocation").has_field("unused_leaves"):
...
def update_leave_allocation_fieldname():
	...
	if dontmanage.db.has_column("Leave Allocation", "carry_forwarded_leaves"):
		dontmanage.db.sql(
			"""
...
def generate_allocation_ledger_entries():
	...
	for allocation in allocation_list:
		if not dontmanage.db.exists(
			...
			allocation_obj = dontmanage.get_doc("Leave Allocation", allocation)
...
def generate_application_leave_ledger_entries():
	...
	for application in leave_applications:
		if not dontmanage.db.exists(
			...
			dontmanage.get_doc("Leave Application", application.name).create_leave_ledger_entry()
...
def generate_encashment_leave_ledger_entries():
	...
	for encashment in leave_encashments:
		if not dontmanage.db.exists(
			...
			dontmanage.get_doc("Leave Encashment", encashment).create_leave_ledger_entry()
...
def get_leaves_application_records():
	return dontmanage.get_all("Leave Application", filters={"docstatus": 1}, fields=["name"])


def get_leave_encashment_records():
	return dontmanage.get_all("Leave Encashment", filters={"docstatus": 1}, fields=["name"])
