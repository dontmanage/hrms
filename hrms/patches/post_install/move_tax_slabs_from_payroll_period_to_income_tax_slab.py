...
def execute():
	if not (
		dontmanage.db.table_exists("Payroll Period") and dontmanage.db.table_exists("Taxable Salary Slab")
	):
		return

	if dontmanage.db.a_row_exists("Income Tax Slab"):
		return
	...
	for doctype in (
		...
		dontmanage.reload_doc("Payroll", "doctype", doctype)

	standard_tax_exemption_amount_exists = dontmanage.db.has_column(
	...
	for company in dontmanage.get_all("Company"):
		payroll_periods = dontmanage.db.sql(
			"""
			SELECT
				{0}
			FROM
	...
	if not dontmanage.db.table_exists("Employee Tax Exemption Proof Submission"):
		return

	if not dontmanage.db.has_column(
		"Employee Tax Exemption Proof Submission", "income_from_other_sources"
	):
		return

	migrated = []
	proofs = dontmanage.get_all(
	...
	for proof in proofs:
		if proof.income_from_other_sources:
			employee_other_income = dontmanage.new_doc("Employee Other Income")
	...
	if not dontmanage.db.table_exists("Employee Tax Exemption Declaration"):
		return

	if not dontmanage.db.has_column("Employee Tax Exemption Declaration", "income_from_other_sources"):
		return

	declerations = dontmanage.get_all(
