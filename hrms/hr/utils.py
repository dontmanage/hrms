...
from dontmanageerp.setup.doctype.employee.employee import (
...
class DuplicateDeclarationError(dontmanage.ValidationError):
	pass


def set_employee_name(doc):
	if doc.employee and not doc.employee_name:
		doc.employee_name = dontmanage.db.get_value("Employee", doc.employee, "employee_name")
...
def update_employee_work_history(employee, details, date=None, cancel=False):
	...
	internal_work_history = {}
	for item in details:
		field = dontmanage.get_meta("Employee").get_field(item.fieldname)
		if not field:
			continue
...
def delete_employee_work_history(details, employee, date):
	...
	if filters:
		dontmanage.db.delete("Employee Internal Work History", filters)
		employee.save()
...
def get_employee_field_property(employee, fieldname):
	if employee and fieldname:
		field = dontmanage.get_meta("Employee").get_field(fieldname)
		value = dontmanage.db.get_value("Employee", employee, fieldname)
...
def validate_dates(doc, from_date, to_date):
	date_of_joining, relieving_date = dontmanage.db.get_value(
		"Employee", doc.employee, ["date_of_joining", "relieving_date"]
	)
	if getdate(from_date) > getdate(to_date):
		dontmanage.throw(_("To date can not be less than from date"))
	elif getdate(from_date) > getdate(nowdate()):
		dontmanage.throw(_("Future dates not allowed"))
	elif date_of_joining and getdate(from_date) < getdate(date_of_joining):
		dontmanage.throw(_("From date can not be less than employee's joining date"))
	elif relieving_date and getdate(to_date) > getdate(relieving_date):
		dontmanage.throw(_("To date can not greater than employee's relieving date"))
...
def validate_duplicate_exemption_for_payroll_period(doctype, docname, payroll_period, employee):
	existing_record = dontmanage.db.exists(
		doctype,
		{
...
def validate_tax_declaration(declarations):
	...
	for d in declarations:
		if d.exemption_sub_category in subcategories:
			dontmanage.throw(_("More than one selection for {0} not allowed").format(d.exemption_sub_category))
		subcategories.append(d.exemption_sub_category)


def get_total_exemption_amount(declarations):
	exemptions = dontmanage._dict()
	for d in declarations:
		exemptions.setdefault(d.exemption_category, dontmanage._dict())
...
def get_leave_period(from_date, to_date, company):
	leave_period = dontmanage.db.sql(
		"""
...
def generate_leave_encashment():
	...
	if dontmanage.db.get_single_value("HR Settings", "auto_leave_encashment"):
		leave_type = dontmanage.get_all("Leave Type", filters={"allow_encashment": 1}, fields=["name"])
		leave_type = [l["name"] for l in leave_type]

		leave_allocation = dontmanage.get_all(
			"Leave Allocation",
...
def allocate_earned_leaves():
	...
	today = dontmanage.flags.current_date or getdate()
	...
	for e_leave_type in e_leave_types:
		...
		for allocation in leave_allocations:
			...
			annual_allocation = dontmanage.db.get_value(
				"Leave Policy Detail",
				filters={"parent": leave_policy, "leave_type": e_leave_type.name},
				fieldname=["annual_allocation"],
			)
			date_of_joining = dontmanage.db.get_value("Employee", allocation.employee, "date_of_joining")
...
def update_previous_leave_allocation(allocation, annual_allocation, e_leave_type, date_of_joining):
	allocation = dontmanage.get_doc("Leave Allocation", allocation.name)
	...
	if (
		...
		today_date = dontmanage.flags.current_date or getdate()
		...
		if e_leave_type.allocate_on_day:
			text = _(
				...
				dontmanage.bold(earned_leaves), dontmanage.bold(formatdate(today_date)), e_leave_type.allocate_on_day
			)
...
def check_effective_date(from_date, today, frequency, allocate_on_day):
	...
	today = dontmanage.flags.current_date or get_datetime(today)
...
def get_salary_assignments(employee, payroll_period):
	...
	assignments = dontmanage.db.get_all(
...
def get_sal_slip_total_benefit_given(employee, payroll_period, component=False):
	...
	sum_of_given_benefit = dontmanage.db.sql(
		query,
		{
			"employee": employee,
...
def get_previous_claimed_amount(employee, payroll_period, non_pro_rata=False, component=False):
	...
	sum_of_claimed_amount = dontmanage.db.sql(
		query,
		{
			"employee": employee,
			"start_date": payroll_period.start_date,
...
def share_doc_with_approver(doc, user):
	...
	if not dontmanage.has_permission(doc=doc, ptype="submit", user=user):
		dontmanage.share.add_docshare(
			doc.doctype, doc.name, user, submit=1, flags={"ignore_share_permission": True}
		)

		dontmanage.msgprint(
			_("Shared with the user {0} with {1} access").format(user, dontmanage.bold("submit"), alert=True)
		)
	...
	if doc_before_save:
		...
		if doc_before_save.get(approver) != doc.get(approver):
			dontmanage.share.remove(doc.doctype, doc.name, doc_before_save.get(approver))
...
def validate_active_employee(employee, method=None):
	...
	if employee and dontmanage.db.get_value("Employee", employee, "status") == "Inactive":
		dontmanage.throw(
...
def validate_loan_repay_from_salary(doc, method=None):
	if doc.applicant_type == "Employee" and doc.repay_from_salary:
		...
		if not doc.applicant:
			dontmanage.throw(_("Please select an Applicant"))

		if not doc.company:
			dontmanage.throw(_("Please select a Company"))

		employee_currency = get_employee_currency(doc.applicant)
		company_currency = dontmanageerp.get_company_currency(doc.company)
	...
	if not doc.is_term_loan and doc.repay_from_salary:
		dontmanage.throw(_("Repay From Salary can be selected only for term loans"))
...
def get_ec_matching_query(bank_account, company, amount_condition, from_date=None, to_date=None):
	...
	mode_of_payments = [
		x["parent"]
		for x in dontmanage.db.get_all(
