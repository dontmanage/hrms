...
from dontmanage.tests.test_webform import create_custom_doctype, create_webform
...
class TestExitInterview(unittest.TestCase):
	def setUp(self):
		dontmanage.db.sql("delete from `tabExit Interview`")

	def test_duplicate_interview(self):
		employee = make_employee("employeeexitint1@example.com")
		dontmanage.db.set_value("Employee", employee, "relieving_date", getdate())
		interview = create_exit_interview(employee)

		doc = dontmanage.copy_doc(interview)
		self.assertRaises(dontmanage.DuplicateEntryError, doc.save)

	def test_relieving_date_validation(self):
		employee = make_employee("employeeexitint2@example.com")
		# unset relieving date
		dontmanage.db.set_value("Employee", employee, "relieving_date", None)

		interview = create_exit_interview(employee, save=False)
		self.assertRaises(dontmanage.ValidationError, interview.save)
	...
	def test_interview_date_updated_in_employee_master(self):
		...
		interview.submit()
		self.assertEqual(dontmanage.db.get_value("Employee", employee, "held_on"), interview.date)

		# exit interview reset on cancel
		interview.reload()
		interview.cancel()
		self.assertEqual(dontmanage.db.get_value("Employee", employee, "held_on"), None)
	...
	def test_send_exit_questionnaire(self):
		...
		webform = dontmanage.db.get_all("Web Form", limit=1)
		...
		email_queue = dontmanage.db.get_all("Email Queue", ["name", "message"], limit=1)
...
def create_notification_template():
	template = dontmanage.db.exists("Email Template", _("Exit Questionnaire Notification"))
	if not template:
		base_path = dontmanage.get_app_path("dontmanageerp", "hr", "doctype")
