...
def create_job_applicant(source_name, target_doc=None):
	emp_ref = dontmanage.get_doc("Employee Referral", source_name)
	...
	job_applicant = dontmanage.new_doc("Job Applicant")
...
def create_additional_salary(doc):
	...
	if not dontmanage.db.exists("Additional Salary", {"ref_docname": doc.name}):
		additional_salary = dontmanage.new_doc("Additional Salary")
		additional_salary.employee = doc.referrer
		additional_salary.company = dontmanage.db.get_value("Employee", doc.referrer, "company")
