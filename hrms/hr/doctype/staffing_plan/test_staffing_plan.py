...
class TestStaffingPlan(unittest.TestCase):
	def test_staffing_plan(self):
		_set_up()
		dontmanage.db.set_value("Company", "_Test Company 3", "is_group", 1)
		if dontmanage.db.exists("Staffing Plan", "Test"):
			return
		staffing_plan = dontmanage.new_doc("Staffing Plan")
	...
	def test_staffing_plan_subsidiary_company(self):
		...
		if dontmanage.db.exists("Staffing Plan", "Test 1"):
			return
		staffing_plan = dontmanage.new_doc("Staffing Plan")
...
def make_company(name=None, abbr=None):
	...
	if dontmanage.db.exists("Company", name):
		return

	company = dontmanage.new_doc("Company")
