...
from dontmanage.utils import cint, flt, getdate, nowdate
from dontmanage.utils.nestedset import get_descendants_of


class SubsidiaryCompanyError(dontmanage.ValidationError):
	pass


class ParentCompanyError(dontmanage.ValidationError):
	pass
...
class StaffingPlan(Document):
	...
	def validate_with_parent_plan(self, staffing_plan_detail):
		if not dontmanage.get_cached_value("Company", self.company, "parent_company"):
		...
		parent_company = dontmanage.db.get_value("Staffing Plan", parent_plan_details[0].name, "company")
		...
		if cint(staffing_plan_detail.vacancies) > cint(parent_plan_details[0].vacancies) or flt(
			...
			dontmanage.throw(
				_(
					...
					dontmanage.bold(staffing_plan_detail.designation),
		...
		lft, rgt = dontmanage.get_cached_value("Company", parent_company, ["lft", "rgt"])
		all_sibling_details = dontmanage.db.sql(
	...
	def validate_with_subsidiary_plans(self, staffing_plan_detail):
		...
		children_details = dontmanage.db.sql(
			"""select sum(spd.vacancies) as vacancies,
			sum(spd.total_estimated_cost) as total_estimated_cost
			from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
			where spd.designation=%s and sp.docstatus=1
	...
	def set_job_requisitions(self, job_reqs):
		if job_reqs:
			requisitions = dontmanage.db.get_list(
				"Job Requisition",
...
def get_designation_counts(designation, company, job_opening=None):
	...
	employee_count = dontmanage.db.count(
	...
	job_openings = dontmanage.db.count("Job Opening", filters)
...
def get_active_staffing_plan_details(company, designation, from_date=None, to_date=None):
	...
	if not company or not designation:
		dontmanage.throw(_("Please select Company and Designation"))

	staffing_plan = dontmanage.db.sql(
		"""
	...
	if not staffing_plan:
		parent_company = dontmanage.get_cached_value("Company", company, "parent_company")
		if parent_company:
