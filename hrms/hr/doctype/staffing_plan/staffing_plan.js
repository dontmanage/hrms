...
dontmanage.ui.form.on('Staffing Plan', {
	setup: function(frm) {
		...
		new dontmanage.ui.form.MultiSelectDialog({
			...
			action(selections) {
				...
				dontmanage.call({
					...
					// hack to retain prompt name that gets lost on dontmanage.call
...
dontmanage.ui.form.on('Staffing Plan Detail', {
	...
	vacancies: function(frm, cdt, cdn) {
		...
		if(child.vacancies < child.current_openings) {
			dontmanage.throw(__("Vacancies cannot be lower than the current openings"));
		}
		set_number_of_positions(frm, cdt, cdn);
	},
...
var set_number_of_positions = function(frm, cdt, cdn) {
	let child = locals[cdt][cdn];
	if (!child.designation) dontmanage.throw(__("Please enter the designation"));
	dontmanage.call({
		...
		callback: function (data) {
			if(data.message){
				dontmanage.model.set_value(cdt, cdn, 'current_count', data.message.employee_count);
				dontmanage.model.set_value(cdt, cdn, 'current_openings', data.message.job_openings);
				...
				if (cint(child.number_of_positions) < total_positions){
					dontmanage.model.set_value(cdt, cdn, 'number_of_positions', total_positions);
				}
			}
			else{ // No employees for this designation
				dontmanage.model.set_value(cdt, cdn, 'current_count', 0);
				dontmanage.model.set_value(cdt, cdn, 'current_openings', 0);
			}
		}
	});
	refresh_field("staffing_details");
...
var set_total_estimated_cost = function(frm, cdt, cdn) {
	...
	if(child.vacancies > 0 && child.estimated_cost_per_position) {
		dontmanage.model.set_value(cdt, cdn, 'total_estimated_cost', child.vacancies * child.estimated_cost_per_position);
	}
	else {
		dontmanage.model.set_value(cdt, cdn, 'total_estimated_cost', 0);
	}
