...
from dontmanage.utils import global_date_format
...
class DailyWorkSummary(Document):
	def send_mails(self, dws_group, emails):
		...
		incoming_email_account = dontmanage.db.get_value(
	...
	def get_message_details(self):
		...
		dws_group = dontmanage.get_doc("Daily Work Summary Group", self.daily_work_summary_group)

		replies = dontmanage.get_all(
			"Communication",
		...
		for d in replies:
			...
			try:
				if original_image:
					file_name = dontmanage.get_list("File", {"file_url": original_image})
		...
		did_not_reply = [
			(dontmanage.db.get_value("User", {"email": email}, "full_name") or email) for email in did_not_reply
		]
...
def get_user_emails_from_group(group):
	...
	if isinstance(group_doc, str):
		group_doc = dontmanage.get_doc("Daily Work Summary Group", group)
