...
# test_records = dontmanage.get_test_records('Daily Work Summary')
...
class TestDailyWorkSummary(unittest.TestCase):
	...
	def test_email_trigger_failed(self):
		hour = "00:00"
		if dontmanage.utils.nowtime().split(":")[0] == "00":
			hour = "01:00"
	...
	def test_incoming(self):
		...
		daily_work_summary = dontmanage.get_doc(
			"Daily Work Summary", dontmanage.get_all("Daily Work Summary")[0].name
		)
	...
	def setup_and_prepare_test(self, hour=None):
		dontmanage.db.sql("delete from `tabDaily Work Summary`")
		dontmanage.db.sql("delete from `tabEmail Queue`")
		dontmanage.db.sql("delete from `tabEmail Queue Recipient`")
		dontmanage.db.sql("delete from `tabCommunication`")
		dontmanage.db.sql("delete from `tabDaily Work Summary Group`")

		self.users = dontmanage.get_all(
	...
	def setup_groups(self, hour=None):
		...
		if not hour:
			hour = dontmanage.utils.nowtime().split(":")[0]
			hour = hour + ":00"

		groups = dontmanage.get_doc(
			dict(
