...
class TestEmployeePerformanceFeedback(DontManageTestCase):
	def setUp(self):
		...
		self.appraisal = dontmanage.db.get_all("Appraisal", filters={"appraisal_cycle": cycle.name})[0].name
	...
	def test_validate_employees(self):
		...
		self.assertRaises(dontmanage.ValidationError, feedback.insert)
	...
	def test_update_avg_feedback_score_in_appraisal(self):
		...
		avg_feedback_score = dontmanage.db.get_value("Appraisal", self.appraisal, "avg_feedback_score")
