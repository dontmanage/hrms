...
from dontmanage.utils import flt, get_link_to_form
...
class EmployeePerformanceFeedback(Document):
		...
		if self.employee == self.reviewer:
			dontmanage.throw(
				_("Employees cannot give feedback to themselves. Use {0} instead: {1}").format(
					dontmanage.bold(_("Self Appraisal")), get_link_to_form("Appraisal", self.appraisal)
				)
			)
	...
	def validate_appraisal(self):
		employee = dontmanage.db.get_value("Appraisal", self.appraisal, "employee")
	...
	def update_avg_feedback_score_in_appraisal(self):
		...
		appraisal = dontmanage.get_doc("Appraisal", self.appraisal)
	...
	def set_feedback_criteria(self):
		...
		template = dontmanage.db.get_value("Appraisal", self.appraisal, "appraisal_template")
		template = dontmanage.get_doc("Appraisal Template", template)
