...
class LeaveEncashment(Document):
	...
	def before_submit(self):
		if self.encashment_amount <= 0:
			dontmanage.throw(_("You can only submit Leave Encashment for a valid encashment amount"))
	...
	def on_submit(self):
		...
		additional_salary.company = dontmanage.get_value("Employee", self.employee, "company")
		additional_salary.employee = self.employee
		additional_salary.currency = self.currency
		earning_component = dontmanage.get_value("Leave Type", self.leave_type, "earning_component")
		if not earning_component:
			dontmanage.throw(_("Please set Earning Component for Leave type: {0}.").format(self.leave_type))
		...
		dontmanage.db.set_value(
			...
			dontmanage.db.get_value("Leave Allocation", self.leave_allocation, "total_leaves_encashed")
	...
	def on_cancel(self):
		if self.additional_salary:
			dontmanage.get_doc("Additional Salary", self.additional_salary).cancel()
	...
	def get_leave_details_for_encashment(self):
		...
		if not dontmanage.db.get_value("Leave Type", self.leave_type, "allow_encashment"):
			dontmanage.throw(_("Leave Type {0} is not encashable").format(self.leave_type))
		...
		encashable_days = self.leave_balance - dontmanage.db.get_value(
			"Leave Type", self.leave_type, "encashment_threshold_days"
		)
		self.encashable_days = encashable_days if encashable_days > 0 else 0

		per_day_encashment = dontmanage.db.get_value(
