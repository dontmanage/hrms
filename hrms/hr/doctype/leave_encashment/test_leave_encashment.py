...
from dontmanage.utils import add_days, get_year_ending, get_year_start, getdate
...
test_records = dontmanage.get_test_records("Leave Type")


class TestLeaveEncashment(DontManageTestCase):
	def setUp(self):
		dontmanage.db.delete("Leave Period")
		dontmanage.db.delete("Leave Policy Assignment")
		dontmanage.db.delete("Leave Allocation")
		dontmanage.db.delete("Leave Ledger Entry")
		dontmanage.db.delete("Additional Salary")
		dontmanage.db.delete("Leave Encashment")

		if not dontmanage.db.exists("Leave Type", "_Test Leave Type Encashment"):
			dontmanage.get_doc(test_records[2]).insert()
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee], dontmanage._dict(data)
		)
	...
	def test_leave_balance_value_and_amount(self):
		leave_encashment = dontmanage.get_doc(
			dict(
		...
		# assert links
		add_sal = dontmanage.get_all("Additional Salary", filters={"ref_docname": leave_encashment.name})[0]
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		self.assertFalse(
			dontmanage.db.exists("Leave Ledger Entry", {"transaction_name": leave_encashment.name})
		)
