...
from dontmanage.utils import flt, validate_email_address
...
class JobApplicant(Document):
	def onload(self):
		job_offer = dontmanage.get_all("Job Offer", filters={"job_applicant": self.name})
		if job_offer:
	...
	def autoname(self):
		...
		if dontmanage.db.exists("Job Applicant", self.name):
	...
	def before_insert(self):
		if self.job_title:
			job_opening_status = dontmanage.db.get_value("Job Opening", self.job_title, "status")
	...
	def set_status_for_employee_referral(self):
		emp_ref = dontmanage.get_doc("Employee Referral", self.employee_referral)
...
def create_interview(doc, interview_round):
	...
	round_designation = dontmanage.db.get_value("Interview Round", interview_round, "designation")
...
def get_applicant_to_hire_percentage():
	total_applicants = dontmanage.db.count("Job Applicant")
	total_hired = dontmanage.db.count("Job Applicant", filters={"status": "Accepted"})
