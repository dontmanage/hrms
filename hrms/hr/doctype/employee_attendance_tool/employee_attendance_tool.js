dontmanage.ui.form.on("Employee Attendance Tool", {
	refresh(frm) {
	...
	onload(frm) {
		frm.set_value("date", dontmanage.datetime.get_today());
	},
	...
	show_unmarked_employees(frm, unmarked_employees) {
		...
		frm.employees_multicheck = dontmanage.ui.form.make_control({
	...
	render_datatable(frm, data, summary_wrapper) {
		...
		if (!frm.marked_emp_datatable) {
			...
			frm.marked_emp_datatable = new dontmanage.DataTable(
	...
	mark_attendance(frm) {
		...
		dontmanage.call({
			...
			if (!r.exc) {
				dontmanage.show_alert({ message: __("Attendance marked successfully"), indicator: "green" });
				frm.refresh();
			}
		});
	},
