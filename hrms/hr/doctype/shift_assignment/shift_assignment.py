...
from dontmanage.utils import cstr, get_datetime, get_link_to_form, get_time, getdate, now_datetime
...
class OverlappingShiftError(dontmanage.ValidationError):
	pass
...
class ShiftAssignment(Document):
	...
	def get_overlapping_dates(self):
		...
		shift = dontmanage.qb.DocType("Shift Assignment")
		query = (
			dontmanage.qb.from_(shift)
...
def has_overlapping_timings(shift_1: str, shift_2: str) -> bool:
	...
	"""
	curr_shift = dontmanage.db.get_value("Shift Type", shift_1, ["start_time", "end_time"], as_dict=True)
	overlapping_shift = dontmanage.db.get_value(
...
def get_events(start, end, filters=None):
	...
	employee = dontmanage.db.get_value(
		"Employee", {"user_id": dontmanage.session.user}, ["name", "company"], as_dict=True
	)
	if employee:
...
def add_assignments(start, end, conditions=None):
	...
	records = dontmanage.db.sql(query, {"start_date": start, "end_date": end}, as_dict=True)
	...
	for d in records:
		...
		while daily_event_start <= daily_event_end:
			start_timing = (
				dontmanage.utils.get_datetime(daily_event_start) + shift_timing_map[d.shift_type]["start_time"]
			)
			end_timing = (
				dontmanage.utils.get_datetime(daily_event_start) + shift_timing_map[d.shift_type]["end_time"]
			)
...
def get_shifts_for_date(employee: str, for_timestamp: datetime) -> List[Dict[str, str]]:
	...
	assignment = dontmanage.qb.DocType("Shift Assignment")

	return (
		dontmanage.qb.from_(assignment)
...
def get_employee_shift(
	...
	default_shift = dontmanage.db.get_value("Employee", employee, "default_shift")
...
def is_holiday_date(employee: str, shift_details: Dict) -> bool:
	holiday_list_name = dontmanage.db.get_value(
...
def get_shift_details(shift_type_name: str, for_timestamp: datetime = None) -> Dict:
	...
	shift_type = dontmanage.get_doc("Shift Type", shift_type_name)
