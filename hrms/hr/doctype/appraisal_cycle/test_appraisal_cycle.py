...
class TestAppraisalCycle(DontManageTestCase):
	def setUp(self):
	...
	def test_create_appraisals(self):
		...
		appraisals = dontmanage.db.get_all("Appraisal", filters={"appraisal_cycle": cycle.name})
		self.assertEqual(len(appraisals), 1)

		appraisal = dontmanage.get_doc("Appraisal", appraisals[0].name)
...
def create_appraisal_cycle(**args):
	...
	if dontmanage.db.exists("Appraisal Cycle", name):
		dontmanage.delete_doc("Appraisal Cycle", name, force=True)

	appraisal_cycle = dontmanage.get_doc(
		{
