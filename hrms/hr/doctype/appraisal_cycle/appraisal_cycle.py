...
class AppraisalCycle(Document):
	...
	def set_employees(self):
		...
		if employees:
			...
			self.set("appraisees", [])
			dontmanage.msgprint(_("No employees found for the selected criteria"))
	...
	def get_appraisal_template_map(self):
		designations = dontmanage.get_all("Designation", fields=["name", "appraisal_template"])
		appraisal_templates = dontmanage._dict()
	...
	def show_missing_template_message(self, raise_exception=False):
		...
		).format(f"""<a href='{dontmanage.utils.get_url_to_list("Designation")}'>Designations</a>""")
	...
	def complete_cycle(self):
		...
		draft_appraisals = dontmanage.db.count("Appraisal", {"appraisal_cycle": self.name, "docstatus": 0})

		if draft_appraisals:
			link = dontmanage.utils.get_url_to_list("Appraisal") + f"?status=Draft&appraisal_cycle={self.name}"
			link = f"""<a href="{link}">documents</a>"""

			msg = _("{0} Appraisal(s) are not submitted yet").format(dontmanage.bold(draft_appraisals))
			msg += "<br><br>"
			msg += _("Please submit the {0} before marking the cycle as Completed").format(link)
			dontmanage.throw(msg, title=_("Unsubmitted Appraisals"))
...
def validate_active_appraisal_cycle(appraisal_cycle: str) -> None:
	if dontmanage.db.get_value("Appraisal Cycle", appraisal_cycle, "status") == "Completed":
		msg = _("Cannot create or change transactions against a {0} Appraisal Cycle.").format(
			dontmanage.bold("Completed")
		)
		msg += "<br><br>"
		msg += _("Mark the cycle as {0} if required.").format(dontmanage.bold("In Progress"))
...
def get_appraisal_cycle_summary(cycle_name: str) -> dict:
	summary = dontmanage._dict()

	summary["appraisees"] = dontmanage.db.count(
		"Appraisal", {"appraisal_cycle": cycle_name, "docstatus": ("!=", 2)}
	)
	summary["self_appraisal_pending"] = dontmanage.db.count(
...
def get_employees_without_goals(cycle_name: str) -> int:
	...
	Appraisal = dontmanage.qb.DocType("Appraisal")
...
def get_employees_without_feedback(cycle_name: str) -> int:
	Feedback = dontmanage.qb.DocType("Employee Performance Feedback")
	Appraisal = dontmanage.qb.DocType("Appraisal")
	count = Count("*").as_("count")

	filtered_records = SubQuery(
		dontmanage.qb.from_(Feedback)
