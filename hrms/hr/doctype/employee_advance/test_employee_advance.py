...
class TestEmployeeAdvance(unittest.TestCase):
	...
	def test_partly_claimed_and_returned_status(self):
		...
		entry = dontmanage.get_doc(entry)
		entry.insert()
		entry.submit()
...
def make_payment_entry(advance):
	journal_entry = dontmanage.get_doc(make_bank_entry("Employee Advance", advance.name))
...
def make_employee_advance(employee_name, args=None):
	doc = dontmanage.new_doc("Employee Advance")
	doc.employee = employee_name
	doc.company = "_Test Company"
	doc.purpose = "For site visit"
	doc.currency = dontmanageerp.get_company_currency("_Test company")
	doc.exchange_rate = 1
