...
dontmanage.ui.form.on('Employee Advance', {
	setup: function(frm) {
		...
		frm.set_query("advance_account", function() {
			if (!frm.doc.employee) {
				dontmanage.msgprint(__("Please select employee first"));
			}
			let company_currency = dontmanageerp.get_currency(frm.doc.company);
	...
	refresh: function(frm) {
		if (frm.doc.docstatus === 1 &&
			...
			dontmanage.model.can_create("Expense Claim")
		) {
		...
		if (
			...
			if (frm.doc.repay_unclaimed_amount_from_salary == 0 && dontmanage.model.can_create("Journal Entry")) {
				frm.add_custom_button(__("Return"), function() {
					frm.trigger('make_return_entry');
				}, __('Create'));
			} else if (frm.doc.repay_unclaimed_amount_from_salary == 1 && dontmanage.model.can_create("Additional Salary")) {
	...
	make_expense_claim: function(frm) {
		return dontmanage.call({
			...
			callback: function(r) {
				const doclist = dontmanage.model.sync(r.message);
				dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
		});
	},
