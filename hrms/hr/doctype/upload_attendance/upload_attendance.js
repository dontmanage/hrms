...
hrms.hr.AttendanceControlPanel = class AttendanceControlPanel extends dontmanage.ui.form.Controller {
	onload() {
		this.frm.set_value("att_fr_date", dontmanage.datetime.get_today());
		this.frm.set_value("att_to_date", dontmanage.datetime.get_today());
	}
	...
	get_template() {
		if(!this.frm.doc.att_fr_date || !this.frm.doc.att_to_date) {
			dontmanage.msgprint(__("Attendance From Date and Attendance To Date is mandatory"));
			return;
		}
		window.location.href = repl(dontmanage.request.url +
	...
	setup_import_progress() {
		...
		dontmanage.realtime.on('import_attendance', (data) => {
			if (data.progress) {
