...
from dontmanage.utils import add_days, cstr, date_diff, getdate
...
def get_template():
	if not dontmanage.has_permission("Attendance", "create"):
		raise dontmanage.PermissionError

	args = dontmanage.local.form_dict

	if getdate(args.from_date) > getdate(args.to_date):
		dontmanage.throw(_("To Date should be greater than From Date"))
	...
	try:
		...
		dontmanage.respond_as_web_page("Holiday List Missing", html=e)
		return

	# write out response as a type csv
	dontmanage.response["result"] = cstr(w.getvalue())
	dontmanage.response["type"] = "csv"
	dontmanage.response["doctype"] = "Attendance"


def add_header(w):
	status = ", ".join(
		(dontmanage.get_meta("Attendance").get_field("status").options or "").strip().split("\n")
	)
	w.writerow(["Notes:"])
...
def get_active_employees():
	employees = dontmanage.db.get_all(
		"Employee",
...
def get_naming_series():
	series = dontmanage.get_meta("Attendance").get_field("naming_series").options.strip().split("\n")
	if not series:
		dontmanage.throw(_("Please setup numbering series for Attendance via Setup > Numbering Series"))
	return series[0]
...
def upload():
	...
	rows = read_csv_content(dontmanage.local.uploaded_file)
	if not rows:
		dontmanage.throw(_("Please select a csv file"))
	dontmanage.enqueue(import_attendances, rows=rows, now=True if len(rows) < 200 else False)
...
def import_attendances(rows):
	...
	from dontmanage.utils.csvutils import check_record, import_doc

	for i, row in enumerate(rows):
		if not row:
			continue
		row_idx = i + 5
		d = dontmanage._dict(zip(columns, row))

		d["doctype"] = "Attendance"
		if d.name:
			d["docstatus"] = dontmanage.db.get_value("Attendance", d.name, "docstatus")

		try:
			check_record(d)
			ret.append(import_doc(d, "Attendance", 1, row_idx, submit=True))
			dontmanage.publish_realtime("import_attendance", dict(progress=i, total=len(rows)))
		except AttributeError:
			pass
		except Exception as e:
			error = True
			ret.append("Error for row (#%d) %s : %s" % (row_idx, len(row) > 1 and row[1] or "", cstr(e)))
			dontmanage.errprint(dontmanage.get_traceback())
	...
	dontmanage.publish_realtime("import_attendance", dict(messages=ret, error=error))
