...
class TestEmployeeOnboarding(unittest.TestCase):
	def setUp(self):
		if dontmanage.db.exists("Employee Onboarding", {"employee_name": "Test Researcher"}):
			dontmanage.delete_doc("Employee Onboarding", {"employee_name": "Test Researcher"})

		project = "Employee Onboarding : test@researcher.com"
		dontmanage.db.sql("delete from tabProject where name=%s", project)
		dontmanage.db.sql("delete from tabTask where project=%s", project)

	def test_employee_onboarding_incomplete_task(self):
		onboarding = create_employee_onboarding()

		project_name = dontmanage.db.get_value("Project", onboarding.project, "project_name")
		...
		# complete the task
		project = dontmanage.get_doc("Project", onboarding.project)
		for task in dontmanage.get_all("Task", dict(project=project.name)):
			task = dontmanage.get_doc("Task", task.name)
...
def get_job_applicant():
	if dontmanage.db.exists("Job Applicant", "test@researcher.com"):
		return dontmanage.get_doc("Job Applicant", "test@researcher.com")
	applicant = dontmanage.new_doc("Job Applicant")
...
def get_job_offer(applicant_name):
	job_offer = dontmanage.db.exists("Job Offer", {"job_applicant": applicant_name})
	if job_offer:
		return dontmanage.get_doc("Job Offer", job_offer)
...
def create_employee_onboarding():
	...
	holiday_list = dontmanage.get_doc("Holiday List", holiday_list)
	holiday_list.holidays = []
	holiday_list.save()

	onboarding = dontmanage.new_doc("Employee Onboarding")
