...
from dontmanage.utils import cstr, flt, get_datetime, get_link_to_form


class DuplicateInterviewRoundError(dontmanage.ValidationError):
	pass
...
class Interview(Document):
	...
	def validate_duplicate_interview(self):
		duplicate_interview = dontmanage.db.exists(
			"Interview",
		...
		if duplicate_interview:
			dontmanage.throw(
				_(
					...
					dontmanage.bold(get_link_to_form("Interview", duplicate_interview)),
					dontmanage.bold(self.job_applicant),
				)
			)

	def validate_designation(self):
		applicant_designation = dontmanage.db.get_value("Job Applicant", self.job_applicant, "designation")
		if self.designation:
			if self.designation != applicant_designation:
				dontmanage.throw(
					_(
						...
						self.interview_round, dontmanage.bold(self.designation), applicant_designation
					),
	...
	def validate_overlap(self):
		...
		if overlaps:
			...
			dontmanage.throw(overlapping_details, title=_("Overlap"))
	...
	def reschedule_interview(self, scheduled_on, from_time, to_time):
		...
		dontmanage.msgprint(_("Interview Rescheduled successfully"), indicator="green")


def get_recipients(name, for_feedback=0):
	interview = dontmanage.get_doc("Interview", name)
	...
	if for_feedback:
		...
		recipients.append(dontmanage.db.get_value("Job Applicant", interview.job_applicant, "email_id"))
...
def send_interview_reminder():
	reminder_settings = dontmanage.db.get_value(
		"HR Settings",
		"HR Settings",
	...
	remind_before = cstr(dontmanage.db.get_single_value("HR Settings", "remind_before")) or "01:00:00"
	...
	interview_template = dontmanage.get_doc(
		"Email Template", reminder_settings.interview_reminder_template
	)

	for d in interviews:
		doc = dontmanage.get_doc("Interview", d.name)
		context = doc.as_dict()
		message = dontmanage.render_template(interview_template.response, context)
...
def send_daily_feedback_reminder():
	...
	interview_feedback_template = dontmanage.get_doc(
	...
	for entry in interviews:
		...
		doc = dontmanage.get_doc("Interview", entry.name)
		context = doc.as_dict()

		message = dontmanage.render_template(interview_feedback_template.response, context)
...
def get_expected_skill_set(interview_round):
	return dontmanage.get_all("Expected Skill Set", filters={"parent": interview_round}, fields=["skill"])
...
def create_interview_feedback(data, interview_name, interviewer, job_applicant):
	...
	if dontmanage.session.user != interviewer:
		dontmanage.throw(_("Only Interviewer Are allowed to submit Interview Feedback"))

	interview_feedback = dontmanage.new_doc("Interview Feedback")
...
def get_events(start, end, filters=None):
	...
	"""
	from dontmanage.desk.calendar import get_event_conditions
