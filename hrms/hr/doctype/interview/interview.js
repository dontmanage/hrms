...
dontmanage.ui.form.on('Interview', {
	onload: function (frm) {
	...
	refresh: function (frm) {
		if (frm.doc.docstatus != 2 && !frm.doc.__islocal) {
			...
			if ((allowed_interviewers.includes(dontmanage.session.user))) {
				dontmanage.db.get_value('Interview Feedback', {'interviewer': dontmanage.session.user, 'interview': frm.doc.name, 'docstatus': 1}, 'name', (r) => {
			...
			primary_action: function(values) {
				dontmanage.call({
					...
					args: {
						...
						interviewer: dontmanage.session.user,
	...
	interview_round: async function (frm) {
		...
		let round_data = (await dontmanage.db.get_value('Interview Round', frm.doc.interview_round, 'designation')).message;
	...
	job_applicant: function (frm) {
		if (!frm.doc.interview_round) {
			...
			frm.refresh();
			dontmanage.throw(__('Select Interview Round First'));
		}
	...
	set_designation_and_job_opening: async function (frm) {
		...
		let job_applicant_data = (await dontmanage.db.get_value(
