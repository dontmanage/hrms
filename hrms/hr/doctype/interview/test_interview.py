...
from dontmanage.utils import add_days, get_time, getdate, nowtime
...
class TestInterview(unittest.TestCase):
	...
	def test_notification_on_rescheduling(self):
		...
		dontmanage.db.sql("DELETE FROM `tabEmail Queue`")
	...
	def test_notification_for_scheduling(self):
		...
		email_queue = dontmanage.db.sql("""select * from `tabEmail Queue`""", as_dict=True)
...
def create_interview_round(name, skill_set, interviewers=[], designation=None, save=True):
	...
	interview_round = dontmanage.new_doc("Interview Round")
...
def create_skill_set(skill_set):
	for skill in skill_set:
		if not dontmanage.db.exists("Skill", skill):
			doc = dontmanage.new_doc("Skill")
			doc.skill_name = skill
			doc.save()


def create_interview_type(name="test_interview_type"):
	if dontmanage.db.exists("Interview Type", name):
		return dontmanage.get_doc("Interview Type", name).name
	else:
		doc = dontmanage.new_doc("Interview Type")
		doc.name = name
...
def setup_reminder_settings():
	if not dontmanage.db.exists("Email Template", _("Interview Reminder")):
	...
	if not dontmanage.db.exists("Email Template", _("Interview Feedback Reminder")):
		base_path = dontmanage.get_app_path("dontmanageerp", "hr", "doctype")
		response = dontmanage.read_file(
