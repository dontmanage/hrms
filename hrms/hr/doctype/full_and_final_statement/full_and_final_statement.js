...
dontmanage.ui.form.on('Full and Final Statement', {
	...
	set_queries: function(frm, type) {
		...
		frm.set_query('reference_document', type, function(doc, cdt, cdn) {
			let fnf_doc = dontmanage.get_doc(cdt, cdn);

			dontmanage.model.with_doctype(fnf_doc.reference_document_type, function() {
				if (dontmanage.model.is_tree(fnf_doc.reference_document_type)) {
					filters['is_group'] = 0;
				}

				if (dontmanage.meta.has_field(fnf_doc.reference_document_type, 'company')) {
					filters['company'] = frm.doc.company;
				}

				if (dontmanage.meta.has_field(fnf_doc.reference_document_type, 'employee')) {
...
dontmanage.ui.form.on("Full and Final Outstanding Statement", {
	reference_document: function(frm, cdt, cdn) {
		...
		if (child.reference_document_type && child.reference_document) {
			dontmanage.call({
				...
				callback: function(r) {
					if (r.message) {
						dontmanage.model.set_value(cdt, cdn, "account", r.message[0]);
						dontmanage.model.set_value(cdt, cdn, "amount", r.message[1]);
					}
				}
			});
		}
