...
from dontmanageerp.assets.doctype.asset.test_asset import create_asset_data
...
class TestFullandFinalStatement(unittest.TestCase):
	...
	def tearDown(self):
		dontmanage.db.sql("Delete from `tabFull and Final Statement`")
		dontmanage.db.sql("Delete from `tabAsset`")
		dontmanage.db.sql("Delete from `tabAsset Movement`")
	...
	def test_check_bootstraped_data_asset_movement_and_jv_creation(self):
		...
		dontmanage.db.set_value("Employee", employee, "relieving_date", add_days(today(), 30))
...
def create_full_and_final_statement(employee):
	fnf = dontmanage.new_doc("Full and Final Statement")
...
def create_asset_movement(employee):
	...
	movement = dontmanage.new_doc("Asset Movement")
...
def create_asset():
	...
	asset_name = dontmanage.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
	asset = dontmanage.get_doc("Asset", asset_name)
