...
def get_approvers(doctype, txt, searchfield, start, page_len, filters):
	...
	if not filters.get("employee"):
		dontmanage.throw(_("Please select Employee first."))

	approvers = []
	department_details = {}
	department_list = []
	employee = dontmanage.get_value(
		"Employee",
	...
	if employee_department:
		department_details = dontmanage.db.get_value(
			"Department", {"name": employee_department}, ["lft", "rgt"], as_dict=True
		)
	if department_details:
		department_list = dontmanage.db.sql(
	...
	if filters.get("doctype") == "Leave Application" and employee.leave_approver:
		approvers.append(
			dontmanage.db.get_value("User", employee.leave_approver, ["name", "first_name", "last_name"])
		)

	if filters.get("doctype") == "Expense Claim" and employee.expense_approver:
		approvers.append(
			dontmanage.db.get_value("User", employee.expense_approver, ["name", "first_name", "last_name"])
		)
	...
	if len(approvers) == 0:
		error_msg = _("Please set {0} for the Employee: {1}").format(
			_(field_name), dontmanage.bold(employee.employee_name)
		)
		if department_list:
			error_msg += " " + _("or for Department: {0}").format(dontmanage.bold(employee_department))
		dontmanage.throw(error_msg, title=_("{0} Missing").format(_(field_name)))
