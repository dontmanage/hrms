...
dontmanage.treeview_settings["Goal"] = {
	...
	filters: [
		...
		},
		{
			...
			get_query() {
				const company = dontmanage.treeview_settings["Goal"].page.fields_dict.company.get_value();
	...
	fields: [
		...
		},
		{
			...
			reqd: 1,
			default() {
				const treeview = dontmanage.treeview_settings["Goal"].treeview;
				let employee = (
	...
	onload(treeview) {
		dontmanage.treeview_settings["Goal"].page = {};
		$.extend(dontmanage.treeview_settings["Goal"].page, treeview.page);
		treeview.make_tree();
	...
	post_render(treeview) {
		dontmanage.treeview_settings["Goal"].treeview = {};
		$.extend(dontmanage.treeview_settings["Goal"].treeview, treeview);
	},
	get_label(node) {
				...
				const dialog = new dontmanage.ui.Dialog({
					...
					primary_action: function() {
						...
						return dontmanage.call({
							...
							if (!r.exc && r.message) {
								dontmanage.treeview_settings["Goal"].treeview.tree.load_children(node.parent_node, true);
								...
								dontmanage.msgprint(__("Could not update progress"));
							}
						});
					},
