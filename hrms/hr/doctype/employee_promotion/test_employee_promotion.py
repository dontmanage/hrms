...
class TestEmployeePromotion(DontManageTestCase):
	def setUp(self):
		dontmanage.db.delete("Employee Promotion")

	def test_submit_before_promotion_date(self):
		employee = make_employee("employee@promotions.com")
		promotion = dontmanage.get_doc(
			{
		...
		self.assertRaises(dontmanage.DocstatusTransitionError, promotion.submit)
	...
	def test_employee_history(self):
		for grade in ["L1", "L2"]:
			dontmanage.get_doc({"doctype": "Employee Grade", "__newname": grade}).insert()
