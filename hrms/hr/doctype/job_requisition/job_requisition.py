...
from dontmanage.utils import format_duration, get_link_to_form, time_diff_in_seconds
...
class JobRequisition(Document):
	...
	def validate_duplicates(self):
		duplicate = dontmanage.db.exists(
			"Job Requisition",
			{
		...
		if duplicate:
			dontmanage.throw(
				_("A Job Requisition for {0} requested by {1} already exists: {2}").format(
					dontmanage.bold(self.designation),
					dontmanage.bold(self.requested_by),
	...
	def associate_job_opening(self, job_opening):
		...
		dontmanage.msgprint(
			_("Job Requisition {0} has been associated with Job Opening {1}").format(
				dontmanage.bold(self.name), get_link_to_form("Job Opening", job_opening)
			),
...
def make_job_opening(source_name, target_doc=None):
	def set_missing_values(source, target):
		...
		target.status = "Open"
		target.currency = dontmanage.db.get_value("Company", source.company, "default_currency")
...
def get_avg_time_to_fill(company=None, department=None, designation=None):
	...
	avg_time_to_fill = dontmanage.db.get_all(
		"Job Requisition",
		filters=filters,
