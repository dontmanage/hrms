...
from dontmanage.utils import get_link_to_form, getdate

from hrms.hr.doctype.shift_assignment.shift_assignment import has_overlapping_timings
from hrms.hr.utils import share_doc_with_approver, validate_active_employee


class OverlappingShiftRequestError(dontmanage.ValidationError):
	pass
...
class ShiftRequest(Document):
	...
	def on_submit(self):
		if self.status not in ["Approved", "Rejected"]:
			dontmanage.throw(_("Only Shift Request with status 'Approved' and 'Rejected' can be submitted"))
		if self.status == "Approved":
			assignment_doc = dontmanage.new_doc("Shift Assignment")
			...
			dontmanage.msgprint(
				_("Shift Assignment: {0} created for Employee: {1}").format(
					dontmanage.bold(assignment_doc.name), dontmanage.bold(self.employee)
				)
			)

	def on_cancel(self):
		shift_assignment_list = dontmanage.get_list(
		...
		if shift_assignment_list:
			for shift in shift_assignment_list:
				shift_assignment_doc = dontmanage.get_doc("Shift Assignment", shift["name"])
				shift_assignment_doc.cancel()

	def validate_default_shift(self):
		default_shift = dontmanage.get_value("Employee", self.employee, "default_shift")
		if self.shift_type == default_shift:
			dontmanage.throw(
				_("You can not request for your Default Shift: {0}").format(dontmanage.bold(self.shift_type))
			)

	def validate_approver(self):
		department = dontmanage.get_value("Employee", self.employee, "department")
		shift_approver = dontmanage.get_value("Employee", self.employee, "shift_request_approver")
		...
		if self.approver not in approvers:
			dontmanage.throw(_("Only Approvers can Approve this Request."))
	...
	def get_overlapping_dates(self):
		...
		shift = dontmanage.qb.DocType("Shift Request")
		query = (
	...
	def throw_overlap_error(self, shift_details):
		...
		dontmanage.throw(msg, title=_("Overlapping Shift Requests"), exc=OverlappingShiftRequestError)
