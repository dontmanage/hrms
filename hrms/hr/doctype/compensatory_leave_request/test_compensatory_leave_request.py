...
import dontmanage
from dontmanage.utils import add_days, add_months, today
...
class TestCompensatoryLeaveRequest(unittest.TestCase):
	def setUp(self):
		dontmanage.db.sql(""" delete from `tabCompensatory Leave Request`""")
		dontmanage.db.sql(""" delete from `tabLeave Ledger Entry`""")
		dontmanage.db.sql(""" delete from `tabLeave Allocation`""")
		dontmanage.db.sql(
	...
	def test_leave_allocation_update_on_submit(self):
		...
		leaves_allocated = dontmanage.db.get_value(
			"Leave Allocation",
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_ledger_entry = dontmanage.get_all("Leave Ledger Entry", fields="*", filters=filters)
...
def get_compensatory_leave_request(employee, leave_date=today()):
	prev_comp_leave_req = dontmanage.db.get_value(
	...
	if prev_comp_leave_req:
		return dontmanage.get_doc("Compensatory Leave Request", prev_comp_leave_req)
...
def create_holiday_list():
	if dontmanage.db.exists("Holiday List", "_Test Compensatory Leave"):
		return
