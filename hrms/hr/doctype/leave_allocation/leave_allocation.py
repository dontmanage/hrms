...
from dontmanage.utils import add_days, date_diff, flt, formatdate, get_link_to_form, getdate
...
class BackDatedAllocationError(dontmanage.ValidationError):
	pass


class OverAllocationError(dontmanage.ValidationError):
	pass


class LessAllocationError(dontmanage.ValidationError):
	pass


class ValueMultiplierError(dontmanage.ValidationError):
	pass
...
class LeaveAllocation(Document):
	...
	def validate_leave_allocation_days(self):
		...
		max_leaves_allowed = flt(
			dontmanage.db.get_value("Leave Type", self.leave_type, "max_leaves_allowed")
		)
	...
	# nosemgrep: dontmanage-semgrep-rules.rules.dontmanage-modifying-but-not-comitting
	...
	def get_existing_leave_count(self):
		ledger_entries = dontmanage.get_all(
			"Leave Ledger Entry",
			filters={
	...
	def validate_earned_leave_update(self):
		if self.leave_policy_assignment and dontmanage.db.get_value(
			...
			dontmanage.throw(msg, title=_("Not Allowed"))
	...
	def update_leave_policy_assignments_when_no_allocations_left(self):
		allocations = dontmanage.db.get_list(
			"Leave Allocation",
	...
	def validate_lwp(self):
		if dontmanage.db.get_value("Leave Type", self.leave_type, "is_lwp"):
			dontmanage.throw(
				_("Leave Type {0} cannot be allocated since it is leave without pay").format(self.leave_type)
			)

	def validate_allocation_overlap(self):
		leave_allocation = dontmanage.db.sql(
			"""
			SELECT
				name
	...
	def set_total_leaves_allocated(self):
		...
		if (
			...
			and not dontmanage.db.get_value("Leave Type", self.leave_type, "is_earned_leave")
			and not dontmanage.db.get_value("Leave Type", self.leave_type, "is_compensatory")
		):
			dontmanage.throw(
	...
	def limit_carry_forward_based_on_max_allowed_leaves(self):
		max_leaves_allowed = dontmanage.db.get_value("Leave Type", self.leave_type, "max_leaves_allowed")
	...
	def validate_total_leaves_allocated(self):
		...
		if date_difference < self.total_leaves_allocated:
			if dontmanage.db.get_value("Leave Type", self.leave_type, "allow_over_allocation"):
				dontmanage.msgprint(
	...
	def create_leave_ledger_entry(self, submit=True):
		if self.unused_leaves:
			expiry_days = dontmanage.db.get_value(
...
def get_leave_allocation_for_period(
	...
	Allocation = dontmanage.qb.DocType("Leave Allocation")
	return (
		dontmanage.qb.from_(Allocation)
...
def get_carry_forwarded_leaves(employee, leave_type, date, carry_forward=None):
	...
	if carry_forward and previous_allocation:
		...
		if unused_leaves:
			max_carry_forwarded_leaves = dontmanage.db.get_value(
...
def validate_carry_forward(leave_type):
	if not dontmanage.db.get_value("Leave Type", leave_type, "is_carry_forward"):
		dontmanage.throw(_("Leave Type {0} cannot be carry-forwarded").format(leave_type))
