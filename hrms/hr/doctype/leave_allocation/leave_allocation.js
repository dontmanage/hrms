...
dontmanage.ui.form.on("Leave Allocation", {
	onload: function(frm) {
		// Ignore cancellation of doctype on cancel all.
		frm.ignore_doctypes_on_cancel_all = ["Leave Ledger Entry"];

		if (!frm.doc.from_date) frm.set_value("from_date", dontmanage.datetime.get_today());
	...
	refresh: function(frm) {
		if (frm.doc.docstatus === 1 && frm.doc.expired) {
			var valid_expiry = moment(dontmanage.datetime.get_today()).isBetween(frm.doc.from_date, frm.doc.to_date);
			if (valid_expiry) {
		...
		if (!frm.doc.__islocal && frm.doc.leave_policy_assignment) {
			dontmanage.db.get_value("Leave Type", frm.doc.leave_type, "is_earned_leave", (r) => {
	...
	expire_allocation: function(frm) {
		dontmanage.call({
			...
			args: {
				'allocation': frm.doc,
				'expiry_date': dontmanage.datetime.get_today()
			},
			freeze: true,
			callback: function(r) {
				if (!r.exc) {
					dontmanage.msgprint(__("Allocation Expired!"));
				}
				frm.refresh();
			}
		});
	...
	leave_policy: function(frm) {
		if (frm.doc.leave_policy && frm.doc.leave_type) {
			dontmanage.db.get_value("Leave Policy Detail", {
...
dontmanage.tour["Leave Allocation"] = [
	{
		fieldname: "employee",
		title: "Employee",
