...
from dontmanage.utils import add_days, add_months, getdate, nowdate
...
class TestLeaveAllocation(DontManageTestCase):
	def setUp(self):
		...
		self.employee = dontmanage.get_doc("Employee", emp_id)
	...
	def test_overlapping_allocation(self):
		...
		dontmanage.get_doc(leaves[0]).save()
		self.assertRaises(dontmanage.ValidationError, dontmanage.get_doc(leaves[1]).save)
	...
	def test_validation_for_over_allocation_based_on_leave_setup(self):
		dontmanage.delete_doc_if_exists("Leave Period", "Test Allocation Period")
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		self.assertFalse(
			dontmanage.db.exists("Leave Ledger Entry", {"transaction_name": leave_allocation.name})
		)
	...
	def test_leave_addition_after_submit(self):
		...
		updated_entry = dontmanage.db.get_all(
			"Leave Ledger Entry",
	...
	def test_validation_against_leave_application_after_submit(self):
		...
		self.assertRaises(dontmanage.ValidationError, leave_allocation.submit)
...
def create_leave_allocation(**args):
	...
	employee = dontmanage.get_doc("Employee", emp_id)
