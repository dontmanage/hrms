...
from dontmanageerp.setup.doctype.holiday_list.test_holiday_list import set_holiday_list
...
class TestLeaveApplication(unittest.TestCase):
	def setUp(self):
		...
		dontmanage.db.delete("Attendance", {"employee": "_T-Employee-00001"})
		dontmanage.db.set_value("Employee", "_T-Employee-00001", "holiday_list", "")
		...
		if not dontmanage.db.exists("Leave Type", "_Test Leave Type"):
			dontmanage.get_doc(
	...
	def _clear_applications(self):
		dontmanage.db.sql("""delete from `tabLeave Application`""")

	def get_application(self, doc):
		application = dontmanage.copy_doc(doc)
	...
	def test_validate_application_across_allocations(self):
		...
		dontmanage.delete_doc_if_exists("Leave Type", "Test Leave Validation", force=1)
		leave_type = dontmanage.get_doc(
		...
		self.assertRaises(dontmanage.ValidationError, leave_application.insert)
	...
	def test_insufficient_leave_balance_validation(self):
		...
		dontmanage.db.set_value("Leave Type", "Test Leave Validation", "allow_negative", True)
		make_leave_application(
	...
	def test_attendance_for_include_holidays(self):
		...
		dontmanage.delete_doc_if_exists("Leave Type", "Test Include Holidays", force=1)
		leave_type = dontmanage.get_doc(
		...
		self.assertEqual(dontmanage.db.count("Attendance", {"leave_application": leave_application.name}), 4)
	...
	def test_attendance_update_for_exclude_holidays(self):
		...
		dontmanage.delete_doc_if_exists("Leave Type", "Test Do Not Include Holidays", force=1)
		leave_type = dontmanage.get_doc(
			dict(
		...
		attendance_on_holiday = dontmanage.get_doc(config)
		...
		attendance = dontmanage.get_doc(config)
		...
		self.assertEqual(dontmanage.db.count("Attendance", {"leave_application": leave_application.name}), 3)

		# attendance on holiday deleted
		self.assertFalse(dontmanage.db.exists("Attendance", attendance_on_holiday.name))

		# attendance on non-holiday updated
		self.assertEqual(dontmanage.db.get_value("Attendance", attendance.name, "status"), "On Leave")

	def test_block_list(self):
		self._clear_roles()

		from dontmanage.utils.user import add_role
	...
	def test_optional_leave(self):
		...
		if not dontmanage.db.exists("Holiday List", holiday_list):
			dontmanage.get_doc(
				dict(
		...
		dontmanage.db.set_value("Leave Period", leave_period.name, "optional_holiday_list", holiday_list)
		leave_type = "Test Optional Type"
		if not dontmanage.db.exists("Leave Type", leave_type):
			dontmanage.get_doc(
	...
	def test_leaves_allowed(self):
		employee = get_employee()
		leave_period = get_leave_period()
		dontmanage.delete_doc_if_exists("Leave Type", "Test Leave Type", force=1)
		leave_type = dontmanage.get_doc(
	...
	def test_applicable_after(self):
		...
		date = add_days(nowdate(), -7)
		dontmanage.db.set_value("Employee", employee.name, "date_of_joining", date)
		...
		dontmanage.delete_doc_if_exists("Leave Type", "Test Leave Type 1", force=1)
		leave_type_1 = dontmanage.get_doc(
		...
		dontmanage.db.set_value("Employee", employee.name, "date_of_joining", "2010-01-01")
	...
	def test_current_leave_on_submit(self):
		...
		if not dontmanage.db.exists("Leave Type", leave_type):
			dontmanage.get_doc(dict(leave_type_name=leave_type, doctype="Leave Type")).insert()

		allocation = dontmanage.get_doc(
			dict(
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_application.submit()
		leave_ledger_entry = dontmanage.get_all(
		...
		leave_application.cancel()
		self.assertFalse(
			dontmanage.db.exists("Leave Ledger Entry", {"transaction_name": leave_application.name})
		)
	...
	def test_leave_approver_perms(self):
		...
		employee.save()
		self.assertTrue("Leave Approver" in dontmanage.get_roles(user))
		...
		application.insert()
		self.assertTrue(application.name in dontmanage.share.get_shared("Leave Application", user))

		# check shared doc revoked
		application.reload()
		application.leave_approver = "test@example.com"
		application.save()
		self.assertTrue(application.name not in dontmanage.share.get_shared("Leave Application", user))
	...
	def test_leave_details_with_expired_cf_leaves(self):
		...
		cf_expiry = dontmanage.db.get_value(
...
def set_leave_approver():
	employee = get_employee()
	dept_doc = dontmanage.get_doc("Department", employee.department)
	dept_doc.append("leave_approvers", {"approver": "test@example.com"})
	dept_doc.save(ignore_permissions=True)


def get_leave_period():
	leave_period_name = dontmanage.db.get_value("Leave Period", {"company": "_Test Company"})
	if leave_period_name:
		return dontmanage.get_doc("Leave Period", leave_period_name)
	else:
...
def allocate_leaves(employee, leave_period, leave_type, new_leaves_allocated, eligible_leaves=0):
	allocate_leave = dontmanage.get_doc(
		{
			"doctype": "Leave Allocation",
			"__islocal": 1,
			"employee": employee.name,
