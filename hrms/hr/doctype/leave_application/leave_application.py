...
from dontmanage.query_builder.functions import Max, Min, Sum
...
from dontmanageerp.buying.doctype.supplier_scorecard.supplier_scorecard import daterange
...
class LeaveDayBlockedError(dontmanage.ValidationError):
	pass
...
class AttendanceAlreadyMarkedError(dontmanage.ValidationError):
	pass


class NotAnOptionalHoliday(dontmanage.ValidationError):
	pass


class InsufficientLeaveBalanceError(dontmanage.ValidationError):
	pass


class LeaveAcrossAllocationsError(dontmanage.ValidationError):
	pass
...
class LeaveApplication(Document):
	...
	def validate(self):
		...
		if dontmanage.db.get_value("Leave Type", self.leave_type, "is_optional_leave"):
	...
	def on_update(self):
		if self.status == "Open" and self.docstatus < 1:
			...
			if dontmanage.db.get_single_value("HR Settings", "send_leave_notification"):
	...
	def validate_applicable_after(self):
		if self.leave_type:
			leave_type = dontmanage.get_doc("Leave Type", self.leave_type)
			if leave_type.applicable_after > 0:
				date_of_joining = dontmanage.db.get_value("Employee", self.employee, "date_of_joining")
				...
				if number_of_days >= 0:
					holidays = 0
					if not dontmanage.db.get_value("Leave Type", self.leave_type, "include_holiday"):
	...
	def validate_dates(self):
		if dontmanage.db.get_single_value("HR Settings", "restrict_backdated_leave_application"):
			if self.from_date and getdate(self.from_date) < getdate():
				allowed_role = dontmanage.db.get_single_value(
				...
				if not allowed_role:
					dontmanage.throw(
						_("Backdated Leave Application is restricted. Please set the {} in {}").format(
							dontmanage.bold("Role Allowed to Create Backdated Leave Application"),
		...
		if (
			...
			dontmanage.throw(_("Half Day Date should be between From Date and To Date"))
	...
	def validate_dates_across_allocation(self):
		if dontmanage.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			return
		...
		if not (alloc_on_from_date or alloc_on_to_date):
			dontmanage.throw(_("Application period cannot be outside leave allocation period"))
	...
	def get_allocation_based_on_application_dates(self) -> Tuple[Dict, Dict]:
		...
		def _get_leave_allocation_record(date):
			LeaveAllocation = dontmanage.qb.DocType("Leave Allocation")
			allocation = (
				dontmanage.qb.from_(LeaveAllocation)
	...
	def validate_back_dated_application(self):
		future_allocation = dontmanage.db.sql(
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			attendance_name = dontmanage.db.exists(
			...
			if date in holiday_dates:
				if attendance_name:
					...
					attendance = dontmanage.get_doc("Attendance", attendance_name)
					attendance.flags.ignore_permissions = True
					if attendance.docstatus == 1:
						attendance.cancel()
					dontmanage.delete_doc("Attendance", attendance_name, force=1)
				continue
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			...
			for name in attendance:
				dontmanage.db.set_value("Attendance", name, "docstatus", 2)

	def validate_salary_processed_days(self):
		if not dontmanage.db.get_value("Leave Type", self.leave_type, "is_lwp"):
			return

		last_processed_pay_slip = dontmanage.db.sql(
			"""
	...
	def show_block_day_warning(self):
		...
		if block_dates:
			dontmanage.msgprint(_("Warning: Leave application contains following block dates") + ":")
			for d in block_dates:
				dontmanage.msgprint(formatdate(d.block_date) + ": " + d.reason)
	...
	def validate_block_days(self):
		...
		if block_dates and self.status == "Approved":
			dontmanage.throw(_("You are not authorized to approve leaves on Block Dates"), LeaveDayBlockedError)
		...
		if dontmanage.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			if leave_balance_for_consumption != self.leave_balance:
				msg = _("Warning: Insufficient leave balance for Leave Type {0} in this allocation.").format(
					dontmanage.bold(self.leave_type)
				)
				msg += "<br><br>"
				msg += _(
			...
			dontmanage.msgprint(msg, title=_("Warning"), indicator="orange")
		else:
			dontmanage.throw(
				_("Insufficient leave balance for Leave Type {0}").format(dontmanage.bold(self.leave_type)),
	...
	def throw_overlap_error(self, d):
		...
		dontmanage.throw(msg, OverlapError)

	def get_total_leaves_on_half_day(self):
		leave_count_on_half_day_date = dontmanage.db.sql(
	...
	def validate_max_days(self):
		max_days = dontmanage.db.get_value("Leave Type", self.leave_type, "max_continuous_days_allowed")
		if max_days and self.total_leave_days > cint(max_days):
			dontmanage.throw(_("Leave of type {0} cannot be longer than {1}").format(self.leave_type, max_days))
	...
	def validate_optional_leave(self):
		...
		if not leave_period:
			dontmanage.throw(_("Cannot find active Leave Period"))
		optional_holiday_list = dontmanage.db.get_value(
	...
	def notify_employee(self):
		...
		parent_doc = dontmanage.get_doc("Leave Application", self.name)
		args = parent_doc.as_dict()

		template = dontmanage.db.get_single_value("HR Settings", "leave_status_notification_template")
		if not template:
			dontmanage.msgprint(_("Please set default template for Leave Status Notification in HR Settings."))
			return
		email_template = dontmanage.get_doc("Email Template", template)
		message = dontmanage.render_template(email_template.response, args)
	...
	def notify_leave_approver(self):
		if self.leave_approver:
			...
			template = dontmanage.db.get_single_value("HR Settings", "leave_approval_notification_template")
			if not template:
				dontmanage.msgprint(
		...
		if cint(self.follow_via_email):
			...
			if not isinstance(contact, list):
				if not args.notify == "employee":
					contact = dontmanage.get_doc("User", contact).email or contact

			sender = dict()
			sender["email"] = dontmanage.get_doc("User", dontmanage.session.user).email
			...
			try:
				...
				dontmanage.msgprint(_("Email sent to {0}").format(contact))
	...
	def create_leave_ledger_entry(self, submit=True):
		...
		lwp = dontmanage.db.get_value("Leave Type", self.leave_type, "is_lwp")
		...
		if expiry_date:
			...
			if self.is_separate_ledger_entry_required(alloc_on_from_date, alloc_on_to_date):
				...
				raise_exception = False if dontmanage.flags.in_patch else True
				args = dict(
...
def get_number_of_leave_days(
	...
	if not dontmanage.db.get_value("Leave Type", leave_type, "include_holiday"):
...
def get_leave_details(employee, date):
	...
	leave_allocation = {}
	precision = cint(dontmanage.db.get_single_value("System Settings", "float_precision", cache=True))

	for d in allocation_records:
		allocation = allocation_records.get(d, dontmanage._dict())
	...
	lwp = dontmanage.get_list("Leave Type", filters={"is_lwp": 1}, pluck="name")
...
def get_leave_balance_on(
	...
	allocation = allocation_records.get(leave_type, dontmanage._dict())
...
def get_leave_allocation_records(employee, date, leave_type=None):
	...
	Ledger = dontmanage.qb.DocType("Leave Ledger Entry")
	LeaveAllocation = dontmanage.qb.DocType("Leave Allocation")

	cf_leave_case = (
		dontmanage.qb.terms.Case().when(Ledger.is_carry_forward == "1", Ledger.leaves).else_(0)
	)
	sum_cf_leaves = Sum(cf_leave_case).as_("cf_leaves")

	new_leaves_case = (
		dontmanage.qb.terms.Case().when(Ledger.is_carry_forward == "0", Ledger.leaves).else_(0)
	)
	sum_new_leaves = Sum(new_leaves_case).as_("new_leaves")

	query = (
		dontmanage.qb.from_(Ledger)
	...
	allocated_leaves = dontmanage._dict()
...
def get_remaining_leaves(
	...
	return dontmanage._dict(leave_balance=leave_balance, leave_balance_for_consumption=remaining_leaves)
	...
	for leave_entry in leave_entries:
		...
		if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
			...
			if leave_entry.leaves % 1:
				half_day = 1
				half_day_date = dontmanage.db.get_value(
...
def is_lwp(leave_type):
	lwp = dontmanage.db.sql("select is_lwp from `tabLeave Type` where name = %s", leave_type)
...
def get_events(start, end, filters=None):
	...
	employee = dontmanage.db.get_value(
		"Employee", filters={"user_id": dontmanage.session.user}, fieldname=["name", "company"], as_dict=True
	)
	...
	if employee:
		...
		employee = ""
		company = dontmanage.db.get_value("Global Defaults", None, "default_company")

	conditions = get_filters_cond("Leave Application", filters, [])
	# show department leaves for employee
	if "Employee" in dontmanage.get_roles():
...
def add_department_leaves(events, start, end, employee, company):
	department = dontmanage.db.get_value("Employee", employee, "department")

	if not department:
		return

	# department leaves
	department_employees = dontmanage.db.sql_list(
...
def add_leaves(events, start, end, filter_conditions=None):
	...
	if not cint(
		dontmanage.db.get_value("HR Settings", None, "show_leaves_of_all_department_members_in_calendar")
	):
	...
	for d in dontmanage.db.sql(query, {"start": start, "end": end}, as_dict=True):
		e = {
			"name": d.name,
...
def add_holidays(events, start, end, employee, company):
	...
	for holiday in dontmanage.db.sql(
...
def get_mandatory_approval(doctype):
	...
	if doctype == "Leave Application":
		mandatory = dontmanage.db.get_single_value(
...
def get_leave_approver(employee):
	leave_approver, department = dontmanage.db.get_value(
		"Employee", employee, ["leave_approver", "department"]
	)

	if not leave_approver and department:
		leave_approver = dontmanage.db.get_value(
			"Department Approver",
