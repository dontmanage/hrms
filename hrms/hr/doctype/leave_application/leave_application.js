...
dontmanage.ui.form.on("Leave Application", {
	setup: function(frm) {
	...
	onload: function(frm) {
		...
		if (!frm.doc.posting_date) {
			frm.set_value("posting_date", dontmanage.datetime.get_today());
		}
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.employee) {
			...
			frm.dashboard.add_section(
				dontmanage.render_template('leave_application_dashboard', {
					data: leave_details
				}),
				__("Allocated Leaves")
			);
	...
	refresh: function(frm) {
		...
		cur_frm.set_intro("");
		if (frm.doc.__islocal && !in_list(dontmanage.user_roles, "Employee")) {
			frm.set_intro(__("Fill the form and save it"));
		}

		if (!frm.doc.employee && dontmanage.defaults.get_user_permissions()) {
			const perm = dontmanage.defaults.get_user_permissions();
	...
	leave_approver: function(frm) {
		if (frm.doc.leave_approver) {
			frm.set_value("leave_approver_name", dontmanage.user.full_name(frm.doc.leave_approver));
		}
	},
	...
	half_day_datepicker: function(frm) {
		...
		half_day_datepicker.update({
			minDate: dontmanage.datetime.str_to_obj(frm.doc.from_date),
			maxDate: dontmanage.datetime.str_to_obj(frm.doc.to_date)
		});
	},
	...
	calculate_total_days: function(frm) {
		if (frm.doc.from_date && frm.doc.to_date && frm.doc.employee && frm.doc.leave_type) {
			...
			if (to_date < from_date) {
				dontmanage.msgprint(__("To Date cannot be less than From Date"));
...
dontmanage.tour["Leave Application"] = [
	{
		fieldname: "employee",
		title: "Employee",
