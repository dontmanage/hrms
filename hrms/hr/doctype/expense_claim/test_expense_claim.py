...
from dontmanage.utils import flt, nowdate, random_string
...
class TestExpenseClaim(DontManageTestCase):
	def setUp(self):
		if not dontmanage.db.get_value("Cost Center", {"company": company_name}):
			dontmanage.get_doc(
				{
					"doctype": "Cost Center",
	...
	def test_total_expense_claim_for_project(self):
		dontmanage.db.sql("""delete from `tabTask`""")
		dontmanage.db.sql("""delete from `tabProject`""")
		dontmanage.db.sql("update `tabExpense Claim` set project = '', task = ''")

		project = dontmanage.get_doc({"project_name": "_Test Project 1", "doctype": "Project"})
		project.save()

		task = dontmanage.get_doc(
		...
		self.assertEqual(dontmanage.db.get_value("Task", task_name, "total_expense_claim"), 200)
		self.assertEqual(dontmanage.db.get_value("Project", project.name, "total_expense_claim"), 200)

		expense_claim2 = make_expense_claim(
			payable_account, 600, 500, company_name, "Travel Expenses - _TC3", project.name, task_name
		)

		self.assertEqual(dontmanage.db.get_value("Task", task_name, "total_expense_claim"), 700)
		self.assertEqual(dontmanage.db.get_value("Project", project.name, "total_expense_claim"), 700)
	...
	def test_expense_claim_against_fully_paid_advances(self):
		...
		dontmanage.db.delete("Employee Advance")
	...
	def test_rejected_expense_claim(self):
		payable_account = get_payable_account(company_name)
		expense_claim = dontmanage.get_doc(
			{
	...
	def test_expense_approver_perms(self):
		...
		expense_claim.save()
		self.assertTrue(expense_claim.name in dontmanage.share.get_shared("Expense Claim", user))

		# check shared doc revoked
		expense_claim.reload()
		expense_claim.expense_approver = "test@example.com"
		expense_claim.save()
		self.assertTrue(expense_claim.name not in dontmanage.share.get_shared("Expense Claim", user))
	...
	def test_expense_claim_against_delivery_trip(self):
		from dontmanageerp.stock.doctype.delivery_trip.test_delivery_trip import (
			create_address,
...
def get_payable_account(company):
	return dontmanage.get_cached_value("Company", company, "default_payable_account")
...
def make_expense_claim(
	...
	if not employee:
		employee = dontmanage.db.get_value("Employee", {"status": "Active", "company": company})
		if not employee:
			employee = make_employee("test_employee@expenseclaim.com", company=company)

	currency, cost_center = dontmanage.db.get_value(
	...
	expense_claim = dontmanage.get_doc(expense_claim)
...
def get_outstanding_and_total_reimbursed_amounts(expense_claim):
	outstanding_amount = flt(
		dontmanage.db.get_value("Expense Claim", expense_claim.name, "total_sanctioned_amount")
	) - flt(dontmanage.db.get_value("Expense Claim", expense_claim.name, "total_amount_reimbursed"))
	total_amount_reimbursed = flt(
		dontmanage.db.get_value("Expense Claim", expense_claim.name, "total_amount_reimbursed")
	)
