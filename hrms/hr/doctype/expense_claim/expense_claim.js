...
dontmanage.provide("hrms.hr");
dontmanage.provide("dontmanageerp.accounts.dimensions");

dontmanage.ui.form.on('Expense Claim', {
	onload: function(frm) {
...
dontmanage.ui.form.on('Expense Claim Detail', {
	expense_type: function(frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		if (!frm.doc.company) {
			d.expense_type = "";
			dontmanage.msgprint(__("Please set the Company"));
...
cur_frm.cscript.onload = function(doc) {
	if (doc.__islocal) {
		cur_frm.set_value("posting_date", dontmanage.datetime.get_today());
...
cur_frm.cscript.refresh = function(doc) {
	...
	if(!doc.__islocal) {
		...
		if (doc.docstatus===1) {
			...
			if (cint(doc.total_amount_reimbursed) > 0 && dontmanage.model.can_read(entry_doctype)) {
				cur_frm.add_custom_button(__('Bank Entries'), function() {
					...
					dontmanage.set_route("List", entry_doctype);
				}, __("View"));
			}
			/* eslint-enable */
		}
...
cur_frm.cscript.set_help = function(doc) {
	cur_frm.set_intro("");
	if(doc.__islocal && !in_list(dontmanage.user_roles, "HR User")) {
...
dontmanage.ui.form.on("Expense Claim", {
	...
	refresh: function(frm) {
		...
		if (
			frm.doc.docstatus === 1
			&& frm.doc.status !== "Paid"
			&& dontmanage.model.can_create("Payment Entry")
		) {
	...
	make_payment_entry: function(frm) {
		...
		return dontmanage.call({
			...
			callback: function(r) {
				var doclist = dontmanage.model.sync(r.message);
				dontmanage.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
		});
	},
	...
	employee_name: function(frm) {
		dontmanageerp.expense_claim.set_title(frm);
	},
	...
	get_advances: function(frm) {
		dontmanage.model.clear_table(frm.doc, "advances");
		if (frm.doc.employee) {
			return dontmanage.call({
				...
				callback: function(r, rt) {
					...
					if(r.message) {
						$.each(r.message, function(i, d) {
							var row = dontmanage.model.add_child(frm.doc, "Expense Claim Advance", "advances");
...
dontmanage.ui.form.on("Expense Claim Detail", {
	amount: function(frm, cdt, cdn) {
		...
		dontmanage.model.set_value(cdt, cdn, 'sanctioned_amount', child.amount);
	},
	...
	cost_center: function(frm, cdt, cdn) {
		dontmanageerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "expenses", "cost_center");
	}
});

dontmanage.ui.form.on("Expense Claim Advance", {
	employee_advance: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if(!frm.doc.employee){
			dontmanage.msgprint(__('Select an employee to get the employee advance.'));
			frm.doc.advances = [];
			refresh_field("advances");
		}
		else {
...
dontmanage.ui.form.on("Expense Taxes and Charges", {
