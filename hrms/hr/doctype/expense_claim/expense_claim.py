...
from dontmanage.utils import cstr, flt, get_link_to_form

import dontmanageerp
from dontmanageerp.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
...
class InvalidExpenseApproverError(dontmanage.ValidationError):
	pass


class ExpenseApproverIdentityError(dontmanage.ValidationError):
	pass


class ExpenseClaim(AccountsController):
	def onload(self):
		self.get("__onload").make_payment_via_journal_entry = dontmanage.db.get_single_value(
	...
	def validate(self):
		...
		if self.task and not self.project:
			self.project = dontmanage.db.get_value("Task", self.task, "project")
	...
	def set_payable_account(self):
		if not self.payable_account and not self.is_paid:
			self.payable_account = dontmanage.get_cached_value(
				"Company", self.company, "default_expense_claim_payable_account"
			)

	def set_cost_center(self):
		if not self.cost_center:
			self.cost_center = dontmanage.get_cached_value("Company", self.company, "cost_center")

	def on_submit(self):
		if self.approval_status == "Draft":
			dontmanage.throw(_("""Approval Status must be 'Approved' or 'Rejected'"""))
	...
	def update_claimed_amount_in_employee_advance(self):
		for d in self.get("advances"):
			dontmanage.get_doc("Employee Advance", d.employee_advance).update_claimed_amount()

	def update_task_and_project(self):
		if self.task:
			task = dontmanage.get_doc("Task", self.task)

			ExpenseClaim = dontmanage.qb.DocType("Expense Claim")
			task.total_expense_claim = (
				dontmanage.qb.from_(ExpenseClaim)
			...
			dontmanage.get_doc("Project", self.project).update_project()
			...
			if not data.cost_center:
				dontmanage.throw(
					_("Row {0}: {1} is required in the expenses table to book an expense claim.").format(
						data.idx, dontmanage.bold("Cost Center")
					)
				)

		if self.is_paid:
			if not self.mode_of_payment:
				dontmanage.throw(_("Mode of payment is required to make a payment").format(self.employee))
	...
	def validate_advances(self):
		...
		for d in self.get("advances"):
			ref_doc = dontmanage.db.get_value(
				"Employee Advance",
				d.employee_advance,
		...
		if self.total_advance_amount:
			...
			if flt(self.total_advance_amount, precision) > amount_with_taxes:
				dontmanage.throw(_("Total advance amount cannot be greater than total sanctioned amount"))
...
def update_reimbursed_amount(doc):
	...
	dontmanage.db.set_value("Expense Claim", doc.name, "total_amount_reimbursed", total_amount_reimbursed)
...
def get_total_reimbursed_amount(doc):
	if doc.is_paid:
		...
		amount_via_jv = dontmanage.db.get_value(
			"Journal Entry Account",
			{"reference_name": doc.name, "docstatus": 1},
			"sum(debit_in_account_currency - credit_in_account_currency)",
		)

		amount_via_payment_entry = dontmanage.db.get_value(
...
def make_bank_entry(dt, dn):
	...
	expense_claim = dontmanage.get_doc(dt, dn)
	...
	je.append(
		"accounts",
		{
			...
			"cost_center": dontmanageerp.get_default_cost_center(expense_claim.company),
...
def get_expense_claim_account_and_cost_center(expense_claim_type, company):
	...
	cost_center = dontmanageerp.get_default_cost_center(company)
	...
	if not account:
		dontmanage.throw(
			_("Set the default account for the {0} {1}").format(
				dontmanage.bold("Expense Claim Type"), get_link_to_form("Expense Claim Type", expense_claim_type)
			)
		)
...
def get_advances(employee, advance_id=None):
	...
	query = dontmanage.qb.from_(advance).select(
		advance.name,
...
def get_expense_claim(
	...
	default_payable_account = dontmanage.get_cached_value(
		"Company", company, "default_expense_claim_payable_account"
	)
	default_cost_center = dontmanage.get_cached_value("Company", company, "cost_center")

	expense_claim = dontmanage.new_doc("Expense Claim")
...
def update_payment_for_expense_claim(doc, method=None):
	...
	for d in doc.get(payment_table):
		if d.get(doctype_field) == "Expense Claim" and d.reference_name:
			expense_claim = dontmanage.get_doc("Expense Claim", d.reference_name)
			if doc.docstatus == 2:
