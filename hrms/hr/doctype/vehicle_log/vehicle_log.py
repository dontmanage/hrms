...
class VehicleLog(Document):
	...
	def on_submit(self):
		dontmanage.db.set_value("Vehicle", self.license_plate, "last_odometer", self.odometer)

	def on_cancel(self):
		distance_travelled = self.odometer - self.last_odometer
		if distance_travelled > 0:
			updated_odometer_value = (
				int(dontmanage.db.get_value("Vehicle", self.license_plate, "last_odometer")) - distance_travelled
			)
			dontmanage.db.set_value("Vehicle", self.license_plate, "last_odometer", updated_odometer_value)


@dontmanage.whitelist()
def make_expense_claim(docname):
	expense_claim = dontmanage.db.exists("Expense Claim", {"vehicle_log": docname})
	if expense_claim:
		dontmanage.throw(_("Expense Claim {0} already exists for the Vehicle Log").format(expense_claim))

	vehicle_log = dontmanage.get_doc("Vehicle Log", docname)
	...
	if not claim_amount:
		dontmanage.throw(_("No additional expenses has been added"))

	exp_claim = dontmanage.new_doc("Expense Claim")
