...
import dontmanage
from dontmanage.utils import cstr, flt, nowdate, random_string
...
class TestVehicleLog(unittest.TestCase):
	...
	def tearDown(self):
		dontmanage.delete_doc("Vehicle", self.license_plate, force=1)
		dontmanage.delete_doc("Employee", self.employee_id, force=1)
	...
	def test_make_vehicle_log_and_syncing_of_odometer_value(self):
		...
		vehicle = dontmanage.get_doc("Vehicle", self.license_plate)
	...
	def test_vehicle_log_fuel_expense(self):
		...
		vehicle_log.cancel()
		dontmanage.delete_doc("Expense Claim", expense_claim.name)
		dontmanage.delete_doc("Vehicle Log", vehicle_log.name)
...
def make_vehicle_log(license_plate, employee_id, with_services=False):
	vehicle_log = dontmanage.get_doc(
		{
			"doctype": "Vehicle Log",
			"license_plate": cstr(license_plate),
			"employee": employee_id,
