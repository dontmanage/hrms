...
from dontmanage.utils import flt, get_link_to_form, now
...
class Appraisal(Document):
		...
		if duplicate:
			dontmanage.throw(
				_(
					...
					get_link_to_form("Appraisal", duplicate), dontmanage.bold(self.employee_name)
				),
	...
	def set_kra_evaluation_method(self):
		if (
			...
			and (
				dontmanage.db.get_value("Appraisal Cycle", self.appraisal_cycle, "kra_evaluation_method")
				== "Manual Rating"
			)
		):
	...
	def set_appraisal_template(self):
		...
		appraisal_template = dontmanage.db.get_value(
			"Appraisee",
			{
	...
	def set_kras_and_rating_criteria(self):
		...
		template = dontmanage.get_doc("Appraisal Template", self.appraisal_template)
		...
		if self.rate_goals_manually:
			...
			for entry in self.goals:
				if flt(entry.score) > 5:
					dontmanage.throw(_("Row {0}: Goal Score cannot be greater than 5").format(entry.idx))
	...
	def calculate_avg_feedback_score(self, update=False):
		avg_feedback_score = dontmanage.qb.avg(
		...
		feedback = dontmanage.get_doc(
			{
				...
				"added_on": now(),
				"feedback": feedback,
				"reviewer": dontmanage.db.get_value("Employee", {"user_id": dontmanage.session.user}),
			}
		)
...
def get_feedback_history(employee, appraisal):
	...
	data.feedback_history = dontmanage.get_list(
		"Employee Performance Feedback",
	...
	feedback_count = dontmanage.db.count(
		"Employee Performance Feedback",
		filters={
			"appraisal": appraisal,
			"employee": employee,
	...
	data.avg_feedback_score = dontmanage.db.get_value("Appraisal", appraisal, "avg_feedback_score")
