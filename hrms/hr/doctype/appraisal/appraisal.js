...
dontmanage.ui.form.on("Appraisal", {
	refresh(frm) {
			...
			dontmanage.run_serially([
				() => {
					if (frm.doc.__islocal && frm.doc.appraisal_cycle) {
						dontmanage.db.get_value("Appraisal Cycle", frm.doc.appraisal_cycle, "kra_evaluation_method", (r) => {
	...
	add_custom_buttons(frm) {
		frm.add_custom_button(__("View Goals"), function() {
			...
			dontmanage.set_route("Tree", "Goal");
		});
	},

	show_feedback_history(frm) {
		dontmanage.require("performance.bundle.js", () => {
...
dontmanage.ui.form.on("Appraisal Goal", {
	score(frm, cdt, cdn) {
		let d = dontmanage.get_doc(cdt, cdn);

		if (flt(d.score) > 5) {
			dontmanage.msgprint(__("Score must be less than or equal to 5"));
			d.score = 0;
	...
	set_score_earned(frm, cdt, cdn) {
		...
		dontmanage.model.set_value(cdt, cdn, "score_earned", score_earned);
