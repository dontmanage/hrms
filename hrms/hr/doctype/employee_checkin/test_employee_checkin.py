...
class TestEmployeeCheckin(DontManageTestCase):
	def setUp(self):
	...
	def test_mark_attendance_and_link_log(self):
		...
		logs_count = dontmanage.db.count(
		...
		now_date = nowdate()
		dontmanage.db.delete("Attendance", {"employee": employee})
		...
		attendance_count = dontmanage.db.count(
			"Attendance",
	...
	def test_unlink_attendance_on_cancellation(self):
		...
		linked_logs = dontmanage.db.get_all("Employee Checkin", {"attendance": attendance.name})
	...
	def test_calculate_working_hours(self):
		...
		logs_type_1 = [dontmanage._dict(x) for x in logs_type_1]
		logs_type_2 = [dontmanage._dict(x) for x in logs_type_2]
	...
	def test_fetch_shift_based_on_default_shift(self):
		...
		date = getdate()
		dontmanage.db.set_value("Employee", employee, "default_shift", default_shift.name)
