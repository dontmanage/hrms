...
class LeavePolicyAssignment(Document):
	...
	def set_dates(self):
		if self.assignment_based_on == "Leave Period":
			self.effective_from, self.effective_to = dontmanage.db.get_value(
				"Leave Period", self.leave_period, ["from_date", "to_date"]
			)
		elif self.assignment_based_on == "Joining Date":
			self.effective_from = dontmanage.db.get_value("Employee", self.employee, "date_of_joining")

	def validate_policy_assignment_overlap(self):
		leave_policy_assignments = dontmanage.get_all(
	...
	def warn_about_carry_forwarding(self):
		...
		leave_policy = dontmanage.get_doc("Leave Policy", self.leave_policy)
		...
		for policy in leave_policy.leave_policy_details:
			...
			if not leave_type.is_carry_forward:
				...
				).format(dontmanage.bold(get_link_to_form("Leave Type", leave_type.name)))
				dontmanage.msgprint(msg, indicator="orange", alert=True)

	@dontmanage.whitelist()
	def grant_leave_alloc_for_employee(self):
		if self.leaves_allocated:
			dontmanage.throw(_("Leave already have been assigned for this Leave Policy Assignment"))
		else:
			leave_allocations = {}
	...
	def get_new_leaves(self, annual_allocation, leave_details, date_of_joining):
		...
		precision = get_field_precision(
			dontmanage.get_meta("Leave Allocation").get_field("new_leaves_allocated")
		)
	...
	def get_leaves_for_passed_months(self, annual_allocation, leave_details, date_of_joining):
		...
		def _get_current_and_from_date():
			current_date = dontmanage.flags.current_date or getdate()
		...
		def _get_pro_rata_period_end_date(consider_current_month):
			...
			date = getdate(dontmanage.flags.current_date) or getdate()
			if consider_current_month:
...
def create_assignment_for_multiple_employees(employees, data):
	...
	for employee in employees:
		assignment = dontmanage.new_doc("Leave Policy Assignment")
		...
		try:
			assignment.submit()
		except dontmanage.exceptions.ValidationError:
			continue
...
def get_leave_type_details():
	leave_type_details = dontmanage._dict()
