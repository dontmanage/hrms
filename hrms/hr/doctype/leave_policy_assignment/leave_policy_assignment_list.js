dontmanage.listview_settings['Leave Policy Assignment'] = {
	onload: function (list_view) {
		let me = this;
		list_view.page.add_inner_button(__("Bulk Leave Policy Assignment"), function () {
			me.dialog = new dontmanage.ui.form.MultiSelectDialog({
				doctype: "Employee",
				target: cur_list,
				setters: {
					employee_name: '',
	...
	set_effective_date: function () {
		if (cur_dialog.fields_dict.assignment_based_on.value === "Leave Period" && cur_dialog.fields_dict.leave_period.value) {
			dontmanage.model.with_doc("Leave Period", cur_dialog.fields_dict.leave_period.value, function () {
				let from_date = dontmanage.model.get_value("Leave Period", cur_dialog.fields_dict.leave_period.value, "from_date");
				let to_date = dontmanage.model.get_value("Leave Period", cur_dialog.fields_dict.leave_period.value, "to_date");
