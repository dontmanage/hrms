...
from dontmanage.utils import get_first_day, getdate
...
class TestLeavePolicyAssignment(DontManageTestCase):
	def setUp(self):
		for doctype in [
			"Leave Period",
			"Leave Application",
	...
	def test_grant_leaves(self):
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee.name], dontmanage._dict(data)
		)
		self.assertEqual(
			dontmanage.db.get_value("Leave Policy Assignment", leave_policy_assignments[0], "leaves_allocated"),
			1,
		)

		leave_allocation = dontmanage.get_list(
			"Leave Allocation",
			filters={
		...
		leave_alloc_doc = dontmanage.get_doc("Leave Allocation", leave_allocation)
