...
from dontmanage.utils import add_days, get_time, get_year_ending, get_year_start, getdate, now_datetime
...
class TestShiftType(DontManageTestCase):
	def setUp(self):
		dontmanage.db.delete("Shift Type")
		dontmanage.db.delete("Shift Assignment")
		dontmanage.db.delete("Employee Checkin")
		dontmanage.db.delete("Attendance")
	...
	def test_working_hours_threshold_for_absent_and_half_day_2(self):
		...
		attendance = dontmanage.db.get_value("Attendance", {"shift": shift_type.name}, "status")
	...
	def test_skip_marking_absent_on_a_holiday(self):
		...
		shift_type.process_auto_attendance()
		attendance = dontmanage.db.get_value("Attendance", {"employee": employee}, "status")
...
def setup_shift_type(**args):
	...
	if not dontmanage.db.exists("Holiday List", "Employee Checkin Test Holiday List"):
...
def make_shift_assignment(shift_type, employee, start_date, end_date=None):
	shift_assignment = dontmanage.get_doc(
		{
