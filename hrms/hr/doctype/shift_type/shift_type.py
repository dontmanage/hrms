...
from dontmanage.utils import cint, get_datetime, get_time, getdate
...
class ShiftType(Document):
					...
					dontmanage.get_doc(
						{
							...
							"content": dontmanage._("Employee was marked Absent due to missing Employee Checkins."),
						}
	...
	def get_start_and_end_dates(self, employee):
		...
		"""
		date_of_joining, relieving_date, employee_creation = dontmanage.db.get_value(
	...
	def get_assigned_employee(self, from_date=None, consider_default_shift=False):
		...
		assigned_employees = dontmanage.get_all("Shift Assignment", filters=filters, pluck="employee")
	...
	def get_employees_with_default_shift(self, from_date=None):
		default_shift_employees = dontmanage.get_all(
		...
		active_shift_assignments = dontmanage.get_all(
			"Shift Assignment",
			filters=filters,
			pluck="employee",
		)
...
def process_auto_attendance_for_all_shifts():
	shift_list = dontmanage.get_all("Shift Type", "name", {"enable_auto_attendance": "1"}, as_list=True)
	for shift in shift_list:
		doc = dontmanage.get_doc("Shift Type", shift[0])
