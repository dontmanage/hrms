...
class JobOpening(WebsiteGenerator):
	...
	def validate(self):
		if not self.route:
			self.route = dontmanage.scrub(self.job_title).replace("_", "-")
	...
	def validate_current_vacancies(self):
		if not self.staffing_plan:
			...
			self.planned_vacancies = dontmanage.db.get_value(
		...
		if self.staffing_plan and self.planned_vacancies:
			staffing_plan_company = dontmanage.db.get_value("Staffing Plan", self.staffing_plan, "company")

			designation_counts = get_designation_counts(self.designation, self.company, self.name)
			current_count = designation_counts["employee_count"] + designation_counts["job_openings"]

			number_of_positions = dontmanage.db.get_value(
				"Staffing Plan Detail",
				{"parent": self.staffing_plan, "designation": self.designation},
				"number_of_positions",
			)
			...
			if number_of_positions <= current_count:
				dontmanage.throw(
					_(
						...
						dontmanage.bold(self.designation), get_link_to_form("Staffing Plan", self.staffing_plan)
					),
	...
	def update_job_requisition_status(self):
		if self.status == "Closed" and self.job_requisition:
			job_requisition = dontmanage.get_doc("Job Requisition", self.job_requisition)
