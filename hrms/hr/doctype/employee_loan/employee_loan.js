...
dontmanage.ui.form.on('Employee Loan', {
	onload: function (frm) {
	...
	make_jv: function (frm) {
		dontmanage.call({
			...
			callback: function (r) {
				if (r.message)
					var doc = dontmanage.model.sync(r.message)[0];
	...
	mode_of_payment: function (frm) {
		if (frm.doc.mode_of_payment && frm.doc.company) {
			dontmanage.call({
				method: "dontmanageerp.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
				args: {
