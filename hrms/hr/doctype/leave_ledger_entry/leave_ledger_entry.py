...
from dontmanage.utils import DATE_FORMAT, flt, getdate, today


class LeaveLedgerEntry(Document):
	def validate(self):
		if getdate(self.from_date) > getdate(self.to_date):
			dontmanage.throw(_("To date needs to be before from date"))

	def on_cancel(self):
		# allow cancellation of expiry leaves
		if self.is_expired:
			dontmanage.db.set_value("Leave Allocation", self.transaction_name, "expired", 0)
		else:
			dontmanage.throw(_("Only expired allocation can be cancelled"))
...
def validate_leave_allocation_against_leave_application(ledger):
	...
	leave_application_records = dontmanage.db.sql_list(
		"""
...
def get_previous_expiry_ledger_entry(ledger):
	...
	creation_date = dontmanage.db.get_value(
		"Leave Ledger Entry",
		filters={
...
def process_expired_allocation():
	...
	leave_type_records = dontmanage.db.get_values(
	...
	expire_allocation = dontmanage.db.sql(
		"""
		SELECT
...
def expire_allocation(allocation, expiry_date=None):
	...
	dontmanage.db.set_value("Leave Allocation", allocation.name, "expired", 1)
