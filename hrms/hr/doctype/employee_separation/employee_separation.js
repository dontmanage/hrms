...
dontmanage.ui.form.on('Employee Separation', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.employee) {
			frm.add_custom_button(__('Employee'), function() {
				dontmanage.set_route("Form", "Employee", frm.doc.employee);
			},__("View"));
		}
		if (frm.doc.project) {
			frm.add_custom_button(__('Project'), function() {
				dontmanage.set_route("Form", "Project", frm.doc.project);
			},__("View"));
			frm.add_custom_button(__('Task'), function() {
				dontmanage.set_route('List', 'Task', {project: frm.doc.project});
			},__("View"));
		}
	},
				...
				callback: function(r) {
					if (r.message) {
						$.each(r.message, function(i, d) {
							var row = dontmanage.model.add_child(frm.doc, "Employee Boarding Activity", "activities");
							$.extend(row, d);
						});
					}
