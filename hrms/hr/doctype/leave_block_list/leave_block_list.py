	...
	def validate(self):
		...
		for d in self.get("leave_block_list_dates"):
			...
			if d.block_date in dates:
				dontmanage.msgprint(_("Date is repeated") + ":" + d.block_date, raise_exception=1)
...
def get_applicable_block_lists(employee=None, company=None, all_lists=False, leave_type=None):
	...
	if department:
		block_list = dontmanage.db.get_value("Department", department, "leave_block_list")
		block_list_leave_type = dontmanage.db.get_value("Leave Block List", block_list, "leave_type")
	...
	add_block_list(dontmanage.db.get_all("Leave Block List", filters=conditions, pluck="name"))
...
def is_user_in_allow_list(block_list):
	return dontmanage.db.get_value(
		...
		{"parent": block_list, "allow_user": dontmanage.session.user},
		"allow_user",
	)
