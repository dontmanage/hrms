...
import dontmanage
from dontmanage.utils import add_days, flt, getdate
...
class TestInterviewFeedback(unittest.TestCase):
	def test_validation_for_skill_set(self):
		...
		dontmanage.set_user(interviewer)

		self.assertRaises(dontmanage.ValidationError, interview_feedback.save)
	...
	def test_average_ratings_on_feedback_submission_and_cancellation(self):
		...
		avg_on_interview_detail = dontmanage.db.get_value(
			"Interview Detail",
			{
