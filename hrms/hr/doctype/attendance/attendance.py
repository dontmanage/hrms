...
class DuplicateAttendanceError(dontmanage.ValidationError):
	pass


class OverlappingShiftAttendanceError(dontmanage.ValidationError):
	pass


class Attendance(Document):
	def validate(self):
		from dontmanageerp.controllers.status_updater import validate_status
	...
	def validate_attendance_date(self):
		...
		if (
			...
			dontmanage.throw(_("Attendance can not be marked for future dates"))
		elif date_of_joining and getdate(self.attendance_date) < getdate(date_of_joining):
			dontmanage.throw(_("Attendance date can not be less than employee's joining date"))
		...
		if duplicate:
			dontmanage.throw(
				_("Attendance for employee {0} is already marked for the date {1}: {2}").format(
					dontmanage.bold(self.employee),
					dontmanage.bold(self.attendance_date),
		...
		if attendance:
			dontmanage.throw(
				_("Attendance for employee {0} is already marked for an overlapping shift {1}: {2}").format(
					dontmanage.bold(self.employee),
					dontmanage.bold(attendance.shift),
	...
	def validate_employee_status(self):
		if dontmanage.db.get_value("Employee", self.employee, "status") == "Inactive":
			dontmanage.throw(_("Cannot mark attendance for an Inactive employee {0}").format(self.employee))
	...
	def validate_employee(self):
		emp = dontmanage.db.sql(
			"select name from `tabEmployee` where name = %s and status = 'Active'", self.employee
		)
		if not emp:
			dontmanage.throw(_("Employee {0} is not active or does not exist").format(self.employee))

	def unlink_attendance_from_checkins(self):
		EmployeeCheckin = dontmanage.qb.DocType("Employee Checkin")
		linked_logs = (
			dontmanage.qb.from_(EmployeeCheckin)
		...
		if linked_logs:
			(
				dontmanage.qb.update(EmployeeCheckin)
				.set("attendance", "")
				.where(EmployeeCheckin.attendance == self.name)
			).run()
...
def get_events(start, end, filters=None):
	...
	employee = dontmanage.db.get_value("Employee", {"user_id": dontmanage.session.user})
...
def add_attendance(events, start, end, conditions=None):
	...
	for d in dontmanage.db.sql(query, {"from_date": start, "to_date": end}, as_dict=True):
		e = {
			"name": d.name,
			"doctype": "Attendance",
...
def mark_attendance(
	...
	company = dontmanage.db.get_value("Employee", employee, "company")
	attendance = dontmanage.get_doc(
		{
			"doctype": "Attendance",
			"employee": employee,
...
def mark_bulk_attendance(data):
	...
	company = dontmanage.get_value("Employee", data.employee, "company")
	if not data.unmarked_days:
		dontmanage.throw(_("Please select a date."))
		return
	...
	for date in data.unmarked_days:
		...
		attendance = dontmanage.get_doc(doc_dict).insert()
		attendance.submit()
