...
from dontmanage.query_builder.functions import Count, Extract, Sum
from dontmanage.utils import cint, cstr, getdate
...
def execute(filters: Optional[Filters] = None) -> Tuple:
	...
	if not (filters.month and filters.year):
		dontmanage.throw(_("Please select month and year."))

	attendance_map = get_attendance_map(filters)
	if not attendance_map:
		dontmanage.msgprint(_("No attendance records found."), alert=True, indicator="orange")
		return [], [], None, None
		...
		columns.append(
			{
				...
				"fieldname": dontmanage.scrub(filters.group_by),
				"fieldtype": "Link",
				"options": "Branch",
				"width": 120,
			}
...
def get_columns_for_leave_types() -> List[Dict]:
	leave_types = dontmanage.db.get_all("Leave Type", pluck="name")
	types = []
	for entry in leave_types:
		types.append(
			{"label": entry, "fieldname": dontmanage.scrub(entry), "fieldtype": "Float", "width": 120}
		)
...
def get_data(filters: Filters, attendance_map: Dict) -> List[Dict]:
	...
	if filters.group_by:
		group_by_column = dontmanage.scrub(filters.group_by)
		...
		for value in group_by_param_values:
			...
			if records:
				data.append({group_by_column: dontmanage.bold(value)})
				data.extend(records)
	else:
...
def get_attendance_map(filters: Filters) -> Dict:
	...
	for d in attendance_list:
		attendance_map.setdefault(d.employee, dontmanage._dict()).setdefault(d.shift, dontmanage._dict())
...
def get_employee_related_details(filters: Filters) -> Tuple[Dict, List]:
	...
	"""
	Employee = dontmanage.qb.DocType("Employee")
	query = (
		dontmanage.qb.from_(Employee)
		.select(
			Employee.name,
	...
	if group_by:
		for parameter, employees in groupby(employee_details, key=lambda d: d[group_by]):
			...
			emp_map.setdefault(parameter, dontmanage._dict())
...
def get_holiday_map(filters: Filters) -> Dict[str, List[Dict]]:
	...
	holiday_lists = dontmanage.db.get_all("Holiday List", pluck="name")
	default_holiday_list = dontmanage.get_cached_value("Company", filters.company, "default_holiday_list")
	holiday_lists.append(default_holiday_list)

	holiday_map = dontmanage._dict()
	Holiday = dontmanage.qb.DocType("Holiday")
...
def get_attendance_summary_and_days(employee: str, filters: Filters) -> Tuple[Dict, List]:
	...
	absent_case = dontmanage.qb.terms.Case().when(Attendance.status == "Absent", 1).else_(0)
	sum_absent = Sum(absent_case).as_("total_absent")

	leave_case = dontmanage.qb.terms.Case().when(Attendance.status == "On Leave", 1).else_(0)
	sum_leave = Sum(leave_case).as_("total_leaves")

	half_day_case = dontmanage.qb.terms.Case().when(Attendance.status == "Half Day", 0.5).else_(0)
...
def get_leave_summary(employee: str, filters: Filters) -> Dict[str, float]:
	...
	"""
	Attendance = dontmanage.qb.DocType("Attendance")
	day_case = dontmanage.qb.terms.Case().when(Attendance.status == "Half Day", 0.5).else_(1)
	...
	for d in leave_details:
		leave_type = dontmanage.scrub(d.leave_type)
...
def get_entry_exits_summary(employee: str, filters: Filters) -> Dict[str, float]:
	...
	late_entry_case = dontmanage.qb.terms.Case().when(Attendance.late_entry == "1", "1")
	count_late_entries = Count(late_entry_case).as_("total_late_entries")

	early_exit_case = dontmanage.qb.terms.Case().when(Attendance.early_exit == "1", "1")
...
def get_attendance_years() -> str:
	...
	if year_list:
		...
		year_list = [dontmanage._dict({"year": getdate().year})]
