...
dontmanage.query_reports["Monthly Attendance Sheet"] = {
	"filters": [
		{
			"fieldname": "month",
			"label": __("Month"),
			...
			"default": dontmanage.datetime.str_to_obj(dontmanage.datetime.get_today()).getMonth() + 1
		},
		{
			"fieldname":"year",
			"label": __("Year"),
	...
	onload: function() {
		return  dontmanage.call({
			...
			callback: function(r) {
				var year_filter = dontmanage.query_report.get_filter('year');
	...
	formatter: function(value, row, column, data, default_formatter) {
		...
		const summarized_view = dontmanage.query_report.get_filter_value('summarized_view');
		const group_by = dontmanage.query_report.get_filter_value('group_by');
