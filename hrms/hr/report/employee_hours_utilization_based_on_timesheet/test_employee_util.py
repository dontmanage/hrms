...
from dontmanageerp.projects.doctype.project.test_project import make_project
...
class TestEmployeeUtilization(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		dontmanage.db.set_value("HR Settings", "HR Settings", "standard_working_hours", 9)

	@classmethod
	def create_test_timesheets(cls):
		timesheet1 = dontmanage.new_doc("Timesheet")
		...
		timesheet2 = dontmanage.new_doc("Timesheet")
	...
	def tearDownClass(cls):
		...
		dontmanage.db.sql("DELETE FROM `tabTimesheet` WHERE company='_Test Company'")
		dontmanage.db.sql(f"DELETE FROM `tabProject` WHERE name='{cls.test_project.name}'")
	...
	def test_utilization_report_for_single_employee(self):
		...
		emp1_data = dontmanage.get_doc("Employee", self.test_emp1)
		expected_data = [
			{
				"employee": self.test_emp1,
	...
	def test_utilization_report_for_project(self):
		...
		emp2_data = dontmanage.get_doc("Employee", self.test_emp2)
		expected_data = [
			{
