...
class EmployeeHoursReport:
	...
	def validate_dates(self):
		...
		if self.day_span <= 0:
			dontmanage.throw(_("From Date must come before To Date"))

	def validate_standard_working_hours(self):
		self.standard_working_hours = dontmanage.db.get_single_value("HR Settings", "standard_working_hours")
		if not self.standard_working_hours:
			msg = _(
				...
				dontmanage.bold("Standard Working Hours"),
				dontmanage.utils.get_link_to_form("HR Settings", "HR Settings"),
			)
	...
	def filter_stats_by_department(self):
		filtered_data = dontmanage._dict()
		for emp, data in self.stats_by_employee.items():
	...
	def generate_filtered_time_logs(self):
		...
		self.filtered_time_logs = dontmanage.db.sql(
			"""
	...
	def generate_stats_by_employee(self):
		self.stats_by_employee = dontmanage._dict()

		for emp, hours, is_billable, project in self.filtered_time_logs:
			self.stats_by_employee.setdefault(emp, dontmanage._dict()).setdefault("billed_hours", 0.0)
	...
	def set_employee_department_and_name(self):
		for emp in self.stats_by_employee:
			emp_name = dontmanage.db.get_value("Employee", emp, "employee_name")
			emp_dept = dontmanage.db.get_value("Employee", emp, "department")
