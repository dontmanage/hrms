...
from dontmanage.utils import add_days, cint, flt, getdate
...
def execute(filters: Optional[Filters] = None) -> Tuple:
	if filters.to_date <= filters.from_date:
		dontmanage.throw(_('"From Date" can not be greater than or equal to "To Date"'))
...
def get_data(filters: Filters) -> List:
	...
	for leave_type in leave_types:
		if consolidate_leave_types:
			...
			row = dontmanage._dict({"leave_type": leave_type})
...
def get_department_leave_approver_map(department: Optional[str] = None):
	...
	department_list = dontmanage.get_list(
		"Department",
		filters={"disabled": 0},
	...
	approver_list = dontmanage.get_all(
		"Department Approver",
	...
	for key, group in groupby(employee_data, lambda x: x["employee_name"]):
		for grp in group:
			if grp.closing_balance:
				leaves.append(
					dontmanage._dict({"leave_type": grp.leave_type, "closing_balance": grp.closing_balance})
				)
