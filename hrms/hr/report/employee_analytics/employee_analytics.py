...
def execute(filters=None):
	...
	if not filters["company"]:
		dontmanage.throw(_("{0} is mandatory").format(_("Company")))
...
def get_parameters(filters):
	...
	return dontmanage.db.sql("""select name from `tab""" + parameter + """` """, as_list=1)
...
def get_chart_data(parameters, employees, filters):
	...
	for parameter in parameters:
		if parameter:
			total_employee = dontmanage.db.sql(
				"""select count(*) from
				`tabEmployee` where """
	...
	total_employee = dontmanage.db.count("Employee", {"status": "Active"})
