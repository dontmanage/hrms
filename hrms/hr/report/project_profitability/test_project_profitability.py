...
from dontmanageerp.projects.doctype.timesheet.timesheet import make_sales_invoice
...
class TestProjectProfitability(DontManageTestCase):
	def setUp(self):
		dontmanage.db.sql("delete from `tabTimesheet`")
		...
		if holidays:
			dontmanage.db.set_value("Payroll Settings", None, "include_holidays_in_total_working_days", 1)
		...
		dontmanage.db.set_value("HR Settings", None, "standard_working_hours", 8)
	...
	def test_project_profitability(self):
		...
		row = report[1][0]
		timesheet = dontmanage.get_doc("Timesheet", self.timesheet.name)
		...
		standard_working_hours = dontmanage.db.get_single_value("HR Settings", "standard_working_hours")
