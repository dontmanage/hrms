...
hrms.PerformanceFeedback = class PerformanceFeedback {
	...
	async render_feedback_history(data) {
		...
		const feedback_html = dontmanage.render_template("performance_feedback_history", {
	...
	get_feedback_criteria_data() {
		...
		return new Promise(resolve => {
			dontmanage.db.get_doc("Appraisal Template", me.frm.doc.appraisal_template)
	...
	async can_create() {
		const is_employee = (
			await dontmanage.db.get_value("Employee", {"user_id": dontmanage.session.user}, "name")
		)?.message?.name || false;

		return (is_employee && dontmanage.model.can_create("Employee Performance Feedback"));
	}
};
