...
from dontmanageerp.loan_management.doctype.process_loan_interest_accrual.process_loan_interest_accrual import (
...
class SalarySlip(TransactionBase):
	...
	def validate(self):
		...
		if dontmanage.db.get_single_value("Payroll Settings", "max_working_hours_against_timesheet"):
			max_working_hours = dontmanage.db.get_single_value(
	...
	def set_net_total_in_words(self):
		...
		company_currency = dontmanageerp.get_company_currency(self.company)
	...
	def on_submit(self):
		if self.net_pay < 0:
			dontmanage.throw(_("Net Pay cannot be less than 0"))
		else:
			self.set_status()
			self.update_status(self.name)
			self.make_loan_repayment_entry()
			if (
				dontmanage.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee")
			) and not dontmanage.flags.via_payroll_entry:
				self.email_salary_slip()

		self.update_payment_status_for_gratuity()

	def update_payment_status_for_gratuity(self):
		additional_salary = dontmanage.db.get_all(
			"Additional Salary",
			filters={
		...
		if additional_salary:
			...
			if additional_salary[0].name in [entry.additional_salary for entry in self.earnings]:
				dontmanage.db.set_value("Gratuity", additional_salary[0].ref_docname, "status", status)
	...
	def validate_dates(self, joining_date=None, relieving_date=None):
		if date_diff(self.end_date, self.start_date) < 0:
			dontmanage.throw(_("To date cannot be before From date"))

		if not joining_date:
			joining_date, relieving_date = dontmanage.get_cached_value(
				"Employee", self.employee, ("date_of_joining", "relieving_date")
			)

		if date_diff(self.end_date, joining_date) < 0:
			dontmanage.throw(_("Cannot create Salary Slip for Employee joining after Payroll Period"))

		if relieving_date and date_diff(relieving_date, self.start_date) < 0:
			dontmanage.throw(_("Cannot create Salary Slip for Employee who has left before Payroll Period"))

	def is_rounding_total_disabled(self):
		return cint(dontmanage.db.get_single_value("Payroll Settings", "disable_rounded_total"))

	def check_existing(self):
		if not self.salary_slip_based_on_timesheet:
			ss = dontmanage.qb.DocType("Salary Slip")
			query = (
				dontmanage.qb.from_(ss)
				.select(ss.name)
				.where(
			...
			for data in self.timesheets:
				if dontmanage.db.get_value("Timesheet", data.time_sheet, "status") == "Payrolled":
					dontmanage.throw(
	...
	def set_time_sheet(self):
		if self.salary_slip_based_on_timesheet:
			...
			Timesheet = dontmanage.qb.DocType("Timesheet")
			timesheets = (
				dontmanage.qb.from_(Timesheet)
	...
	def check_sal_struct(self, joining_date, relieving_date):
		ss = dontmanage.qb.DocType("Salary Structure")
		ssa = dontmanage.qb.DocType("Salary Structure Assignment")

		query = (
			dontmanage.qb.from_(ssa)
			.join(ss)
	...
	def get_working_days_details(
		...
		payroll_based_on = dontmanage.db.get_value("Payroll Settings", None, "payroll_based_on")
		include_holidays_in_total_working_days = dontmanage.db.get_single_value(
		...
		if not cint(include_holidays_in_total_working_days):
			...
			if working_days < 0:
				dontmanage.throw(_("There are more holidays than working days this month."))

		if not payroll_based_on:
			dontmanage.throw(_("Please set Payroll based on in Payroll settings"))
		...
		if flt(payment_days) > flt(lwp):
			...
			consider_unmarked_attendance_as = (
				dontmanage.db.get_value("Payroll Settings", None, "consider_unmarked_attendance_as") or "Present"
			)
	...
	def get_unmarked_days(self, include_holidays_in_total_working_days):
		...
		unmarked_days -= dontmanage.get_all(
			"Attendance",
			filters={
	...
	def get_payment_days(self, joining_date, relieving_date, include_holidays_in_total_working_days):
		...
		if relieving_date:
			employee_status = dontmanage.get_cached_value("Employee", self.employee, "status")
			if getdate(self.start_date) <= relieving_date <= getdate(self.end_date):
				end_date = relieving_date
			elif relieving_date < getdate(self.start_date) and employee_status != "Left":
				dontmanage.throw(_("Employee relieved on {0} must be set as 'Left'").format(relieving_date))
	...
	def calculate_lwp_or_ppl_based_on_leave_application(
		...
		daily_wages_fraction_for_half_day = (
			flt(dontmanage.db.get_value("Payroll Settings", None, "daily_wages_fraction_for_half_day")) or 0.5
		)
	...
	def calculate_lwp_ppl_and_absent_days_based_on_attendance(self, holidays, relieving_date):
		...
		leave_types = dontmanage.get_all(
			"Leave Type",
		...
		attendance = dontmanage.qb.DocType("Attendance")

		attendances = (
			dontmanage.qb.from_(attendance)
	...
	def add_earning_for_hourly_wages(self, doc, salary_component, amount):
		...
		if not row_exists:
			wages_row = {
				...
				"abbr": dontmanage.db.get_value("Salary Component", salary_component, "salary_component_abbr"),
	...
	def get_amount_from_formula(self, struct_row, sub_period=1):
		if self.payroll_frequency == "Monthly":
			start_date = dontmanage.utils.add_months(self.start_date, sub_period)
			end_date = dontmanage.utils.add_months(self.end_date, sub_period)
			posting_date = dontmanage.utils.add_months(self.posting_date, sub_period)
			...
			start_date = dontmanage.utils.add_days(self.start_date, days_to_add)
			end_date = dontmanage.utils.add_days(self.end_date, days_to_add)
	...
	def calculate_component_amounts(self, component_type):
		if not getattr(self, "_salary_structure_doc", None):
			self._salary_structure_doc = dontmanage.get_doc("Salary Structure", self.salary_structure)
	...
	def add_structure_components(self, component_type):
		...
		timesheet_component = dontmanage.db.get_value(
	...
	def get_data_for_eval(self):
		...
		employee = dontmanage.get_doc("Employee", self.employee).as_dict()
		...
		salary_structure_assignment = dontmanage.get_value(
		...
		if not salary_structure_assignment:
			dontmanage.throw(
				_(
					...
					dontmanage.bold(formatdate(date_to_validate)),
				)
			)
		...
		salary_components = dontmanage.get_all("Salary Component", fields=["salary_component_abbr"])
	...
	def eval_condition_and_formula(self, d, data):
		try:
			...
			if condition:
				if not dontmanage.safe_eval(condition, self.whitelisted_globals, data):
					return None
			amount = d.amount
			if d.amount_based_on_formula:
				formula = d.formula.strip().replace("\n", " ") if d.formula else None
				if formula:
					amount = flt(dontmanage.safe_eval(formula, self.whitelisted_globals, data), d.precision("amount"))
			if amount:
				data[d.abbr] = amount
			...
			if getdate(payroll_period.end_date) <= getdate(self.end_date):
				...
				if last_benefits:
					for last_benefit in last_benefits:
						last_benefit = dontmanage._dict(last_benefit)
						amount = last_benefit.amount
						self.update_component_row(dontmanage._dict(last_benefit.struct_row), amount, "earnings")
	...
	def add_tax_components(self):
		...
		if not tax_components:
			tax_components = [
				d.name
				for d in dontmanage.get_all("Salary Component", filters={"variable_based_on_taxable_salary": 1})
	...
	def get_income_tax_slabs(self):
		income_tax_slab, ss_assignment_name = dontmanage.db.get_value(
			"Salary Structure Assignment",
		...
		income_tax_slab_doc = dontmanage.get_doc("Income Tax Slab", income_tax_slab)
		if income_tax_slab_doc.disabled:
			dontmanage.throw(_("Income Tax Slab: {0} is disabled").format(income_tax_slab))
	...
	def get_salary_slip_details(
		...
		ss = dontmanage.qb.DocType("Salary Slip")
		sd = dontmanage.qb.DocType("Salary Detail")
	...
	def get_future_recurring_period(self, additional_salary):
		to_date = dontmanage.db.get_value("Additional Salary", additional_salary, "to_date")
	...
	def calculate_unclaimed_taxable_benefits(self):
		...
		BenefitClaim = dontmanage.qb.DocType("Employee Benefit Claim")
		total_benefits_claimed = (
			dontmanage.qb.from_(BenefitClaim)
	...
	def get_total_exemption_amount(self):
		...
		if self.tax_slab.allow_tax_exemption:
			if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
				exemption_proof = dontmanage.db.get_value(
				...
				declaration = dontmanage.db.get_value(
	...
	def get_joining_and_relieving_dates(self):
		...
		if not joining_date:
			dontmanage.throw(
				_("Please set the Date Of Joining for employee {0}").format(dontmanage.bold(self.employee_name))
			)
			...
			if payment.total_payment > total_amount:
				dontmanage.throw(
					_(
						...
						payment.idx,
						dontmanage.bold(payment.total_payment),
						dontmanage.bold(total_amount),
						dontmanage.bold(payment.loan),
					)
				)
	...
	def get_loan_details(self):
		loan_details = dontmanage.get_all(
			"Loan",
	...
	def cancel_loan_repayment_entry(self):
		for loan in self.loans:
			if loan.loan_repayment_entry:
				repayment_entry = dontmanage.get_doc("Loan Repayment", loan.loan_repayment_entry)
				repayment_entry.cancel()

	def email_salary_slip(self):
		receiver = dontmanage.db.get_value("Employee", self.employee, "prefered_email")
		payroll_settings = dontmanage.get_single("Payroll Settings")
		...
		if receiver:
			email_args = {
				...
				"attachments": [
					dontmanage.attach_print(self.doctype, self.name, file_name=self.name, password=password)
				],
				"reference_doctype": self.doctype,
				"reference_name": self.name,
			}
			if not dontmanage.flags.in_test:
				enqueue(method=dontmanage.sendmail, queue="short", timeout=300, is_async=True, **email_args)
			else:
	...
	def update_status(self, salary_slip=None):
		for data in self.timesheets:
			if data.time_sheet:
				timesheet = dontmanage.get_doc("Timesheet", data.time_sheet)
	...
	def calculate_total_for_salary_slip_based_on_timesheet(self):
		...
		self.base_hour_rate = flt(self.hour_rate) * flt(self.exchange_rate)
		salary_component = dontmanage.db.get_value(
	...
	def compute_year_to_date(self):
		...
		salary_slip_sum = dontmanage.get_list(
			"Salary Slip",
	...
	def add_leave_balances(self):
		...
		if dontmanage.db.get_single_value("Payroll Settings", "show_leave_balances_in_salary_slip"):
...
def unlink_ref_doc_from_salary_slip(doc, method=None):
	...
	linked_ss = dontmanage.get_all(
		"Salary Slip", filters={"journal_entry": doc.name, "docstatus": ["<", 2]}, pluck="name"
	)

	if linked_ss:
		for ss in linked_ss:
			ss_doc = dontmanage.get_doc("Salary Slip", ss)
			dontmanage.db.set_value("Salary Slip", ss_doc.name, "journal_entry", "")
...
def eval_tax_slab_condition(condition, eval_globals=None, eval_locals=None):
	...
	try:
		...
		if condition:
			return dontmanage.safe_eval(condition, eval_globals, eval_locals)
	except NameError as err:
		dontmanage.throw(
		...
		dontmanage.throw(_("Syntax error in condition: {0} in Income Tax Slab").format(err))
	except Exception as e:
		dontmanage.throw(_("Error in formula or condition: {0} in Income Tax Slab").format(e))
		raise


def get_lwp_or_ppl_for_date(date, employee, holidays):
	LeaveApplication = dontmanage.qb.DocType("Leave Application")
	...
	query = (
		dontmanage.qb.from_(LeaveApplication)
...
def make_salary_slip_from_timesheet(source_name, target_doc=None):
	target = dontmanage.new_doc("Salary Slip")
...
def set_missing_values(time_sheet, target):
	doc = dontmanage.get_doc("Timesheet", time_sheet)
...
def throw_error_message(row, error, title, description=None):
	...
	dontmanage.throw(message, title=title)
