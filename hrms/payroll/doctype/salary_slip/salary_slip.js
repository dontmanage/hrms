...
dontmanage.ui.form.on("Salary Slip", {
	setup: function(frm) {
		...
		frm.set_query("employee", function() {
			return {
				query: "dontmanageerp.controllers.queries.employee_query",
				filters: {
	...
	set_dynamic_labels: function(frm) {
		var company_currency = frm.doc.company? dontmanageerp.get_currency(frm.doc.company): dontmanage.defaults.get_default("currency");
		...
		if (frm.doc.docstatus === 0) {
			if (frm.doc.currency) {
				...
				if (from_currency != company_currency) {
					...
					dontmanage.call({
						method: "dontmanageerp.setup.utils.get_exchange_rate",
						args: {
			...
			return dontmanage.call({
				...
				callback: function(r) {
					if (r.message[1] !== "Leave" && r.message[0]) {
						frm.fields_dict.absent_days.set_description(__("Unmarked Days is treated as {0}. You can can change this in {1}", [r.message, dontmanage.utils.get_form_link("Payroll Settings", "Payroll Settings", true)]));
					}
					frm.refresh();
				}
			});
...
dontmanage.ui.form.on('Salary Slip Timesheet', {
