...
from dontmanage.utils.make_random import get_random
...
class TestSalarySlip(DontManageTestCase):
	def setUp(self):
		setup_test()
		dontmanage.flags.pop("via_payroll_entry", None)

	def tearDown(self):
		dontmanage.db.set_value("Payroll Settings", None, "include_holidays_in_total_working_days", 0)
	...
	def test_payment_days_based_on_attendance(self):
		...
		dontmanage.db.set_value("Employee", emp_id, {"relieving_date": None, "status": "Active"})

		dontmanage.db.set_value("Leave Type", "Leave Without Pay", "include_holiday", 0)
	...
	def test_payment_days_for_mid_joinee_including_holidays(self):
		...
		dontmanage.delete_doc("Salary Slip", new_ss.name, force=True)
		new_ss = make_employee_salary_slip(
			"test_payment_days_based_on_joining_date@salary.com",
			"Monthly",
			"Test Payment Based On Attendence",
	...
	def test_payment_days_in_salary_slip_based_on_timesheet(self):
		from dontmanageerp.projects.doctype.timesheet.test_timesheet import make_timesheet
		...
		dontmanage.db.set_value("Employee", emp, {"relieving_date": None, "status": "Active"})
	...
	def test_payment_days(self):
		...
		if date_of_joining.day > 1:
			self.assertRaises(dontmanage.ValidationError, ss.save)
		...
		dontmanage.db.set_value(
			"Employee",
			dontmanage.get_value("Employee", {"employee_name": "test_payment_days@salary.com"}, "name"),
			"relieving_date",
			None,
		)
		dontmanage.db.set_value(
	...
	def test_employee_salary_slip_read_permission(self):
		...
		dontmanage.set_user("test_employee_salary_slip_read_permission@salary.com")
	...
	def test_email_salary_slip(self):
		...
		email_queue = dontmanage.db.a_row_exists("Email Queue")
		self.assertTrue(email_queue)

	def test_loan_repayment_salary_slip(self):
		from dontmanageerp.loan_management.doctype.loan.test_loan import (
			create_loan,
	...
	def test_payroll_frequency(self):
		fiscal_year = get_fiscal_year(nowdate(), company=dontmanageerp.get_default_company())[0]
	...
	def test_year_to_date_computation(self):
		...
		dontmanage.db.sql("DELETE FROM `tabSalary Slip` where employee_name = 'test_ytd@salary.com'")

		create_salary_slips_for_payroll_period(
			applicant, salary_structure.name, payroll_period, deduct_random=False, num=6
		)

		salary_slips = dontmanage.get_all(
			"Salary Slip",
	...
	def test_component_wise_year_to_date_computation(self):
		...
		# clear salary slip for this employee
		dontmanage.db.sql("DELETE FROM `tabSalary Slip` where employee_name = '%s'" % employee_name)
		...
		year_to_date = dict()
		for slip in salary_slips:
			doc = dontmanage.get_doc("Salary Slip", slip.name)
	...
	def test_tax_for_payroll_period(self):
		...
		dontmanage.db.sql("""delete from `tabPayroll Period`""")
		dontmanage.db.sql("""delete from `tabSalary Component`""")
		...
		for doc in delete_docs:
			dontmanage.db.sql("delete from `tab%s` where employee='%s'" % (doc, employee))
		...
		try:
			...
			raise
		dontmanage.db.sql("""delete from `tabSalary Slip` where employee=%s""", (employee))
		...
		try:
			...
			print("\nSalary Slip - Tax calculation failed on following case\n", data, "\n")
			raise
		dontmanage.db.sql("""delete from `tabAdditional Salary` where employee=%s""", (employee))
	...
	def test_salary_slip_from_timesheet(self):
		...
		timesheet = dontmanage.get_doc("Timesheet", timesheet.name)
	...
	def make_activity_for_employee(self):
		activity_type = dontmanage.get_doc("Activity Type", "_Test Activity Type")
	...
	def test_salary_slip_generation_against_opening_entries_in_ssa(self):
		...
		dontmanage.db.sql("DELETE FROM `tabPayroll Period` where company = '_Test Company'")
		dontmanage.db.sql("DELETE FROM `tabIncome Tax Slab` where currency = 'INR'")
		...
		employee_doc = dontmanage.get_doc("Employee", emp)
		...
		if not dontmanage.db.exists("Salary Structure", salary_structure_name):
		...
		ssa = dontmanage.get_value(
			"Salary Structure Assignment",
			{"employee": emp, "salary_structure": salary_structure_doc.name},
			"name",
		)
		ssa_doc = dontmanage.get_doc("Salary Structure Assignment", ssa)
	...
	def test_income_tax_breakup_fields(self):
		...
		employee_doc = dontmanage.get_cached_doc("Employee", emp)

		payroll_period = dontmanage.get_all("Payroll Period", filters={"company": "_Test Company"}, limit=1)
		payroll_period = dontmanage.get_cached_doc("Payroll Period", payroll_period[0].name)
		create_tax_slab(
	...
	def test_lwp_calculation_based_on_relieving_date(self):
		...
		dontmanage.db.set_value("Employee", emp_id, {"relieving_date": relieving_date, "status": "Left"})
...
def make_employee_salary_slip(user, payroll_frequency, salary_structure=None, posting_date=None):
	...
	salary_slip_name = dontmanage.db.get_value(
		"Salary Slip", {"employee": dontmanage.db.get_value("Employee", {"user_id": user})}
	)
	...
	if not salary_slip_name:
		...
		salary_slip = dontmanage.get_doc("Salary Slip", salary_slip_name)
...
def make_salary_component(salary_components, test_tax, company_list=None):
	for salary_component in salary_components:
		if dontmanage.db.exists("Salary Component", salary_component["salary_component"]):
			dontmanage.delete_doc("Salary Component", salary_component["salary_component"], force=True)
		...
		doc = dontmanage.new_doc("Salary Component")
...
def set_salary_component_account(sal_comp, company_list=None):
	...
	if not isinstance(sal_comp, Document):
		sal_comp = dontmanage.get_doc("Salary Component", sal_comp)

	if not sal_comp.get("accounts"):
		for d in company_list:
			company_abbr = dontmanage.get_cached_value("Company", d, "abbr")
...
def create_account(account_name, company, parent_account, account_type=None):
	...
	account = dontmanage.db.get_value("Account", account_name + " - " + company_abbr)
	if not account:
		dontmanage.get_doc(
			{
				"doctype": "Account",
...
def get_tax_paid_in_period(employee):
	tax_paid_amount = dontmanage.db.sql(
...
def create_proof_submission(employee, payroll_period, amount):
	...
	proof_submission = dontmanage.get_doc(
		{
...
def create_tax_slab(
	...
	if company:
		currency = dontmanageerp.get_company_currency(company)
	...
	income_tax_slab_name = dontmanage.db.get_value("Income Tax Slab", {"currency": currency})

	if not income_tax_slab_name:
		income_tax_slab = dontmanage.new_doc("Income Tax Slab")
...
def make_leave_application(
	...
	leave_application = dontmanage.get_doc(
		dict(
			...
			company=company or dontmanageerp.get_default_company() or "_Test Company",
			status="Approved",
...
def setup_test():
	...
	for dt in [
		...
		dontmanage.db.sql("delete from `tab%s`" % dt)

	make_holiday_list()
	make_payroll_period()

	dontmanage.db.set_value(
		"Company", dontmanageerp.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List"
	)

	dontmanage.db.set_value("Payroll Settings", None, "email_salary_slip_to_employee", 0)
	dontmanage.db.set_value("HR Settings", None, "leave_status_notification_template", None)
	dontmanage.db.set_value("HR Settings", None, "leave_approval_notification_template", None)


def make_payroll_period():
	default_company = dontmanageerp.get_default_company()
	...
	for company in company_based_payroll_period:
		payroll_period = dontmanage.db.get_value(
			"Payroll Period",
			{
				"company": company,
...
def make_holiday_list(list_name=None, from_date=None, to_date=None, add_weekly_offs=True):
	fiscal_year = get_fiscal_year(nowdate(), company=dontmanageerp.get_default_company())
	name = list_name or "Salary Slip Test Holiday List"

	dontmanage.delete_doc_if_exists("Holiday List", name, force=True)

	holiday_list = dontmanage.get_doc(
		{
...
def make_salary_structure_for_payment_days_based_component_dependency(test_statistical_comp=False):
	...
	if dontmanage.db.exists("Salary Structure", salary_structure):
		dontmanage.db.delete("Salary Structure", salary_structure)
	...
	salary_structure_doc = dontmanage.get_doc(details)
...
def make_salary_slip_for_payment_days_dependency_test(employee, salary_structure):
	...
	salary_slip_name = dontmanage.db.get_value("Salary Slip", {"employee": employee.name})
...
def make_salary_structure_for_timesheet(employee, company=None):
	...
	if not dontmanage.db.exists("Salary Component", "Timesheet Component"):
		dontmanage.get_doc(
	...
	if not dontmanage.db.get_value("Salary Structure Assignment", {"employee": employee, "docstatus": 1}):
		dontmanage.db.set_value("Employee", employee, "date_of_joining", add_months(nowdate(), -5))
...
def create_employee_other_income(employee, payroll_period, company):
	other_income = dontmanage.db.get_value(
	...
	if not other_income:
		other_income = dontmanage.get_doc(
			{
