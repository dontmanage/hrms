...
dontmanage.ui.form.on('Payroll Entry', {
	onload: function (frm) {
		if (!frm.doc.posting_date) {
			frm.doc.posting_date = dontmanage.datetime.nowdate();
		}
	...
	refresh: function (frm) {
		...
		if (
			...
			&& !dontmanage.model.has_workflow(frm.doctype)
		...
		dontmanage.realtime.on("completed_salary_slip_creation", function() {
			frm.reload_doc();
		});

		dontmanage.realtime.on("completed_salary_slip_submission", function() {
			frm.reload_doc();
		});
	},
	...
	setup: function (frm) {
		...
		frm.set_query('employee', 'employees', () => {
			...
			mandatory_fields.forEach(field => {
				if (!frm.doc[field]) {
					error_fields.push(dontmanage.unscrub(field));
				}
			});
	...
	set_payable_account_and_currency: function (frm) {
		dontmanage.db.get_value("Company", {"name": frm.doc.company}, "default_currency", (r) => {
			frm.set_value('currency', r.default_currency);
		});
		dontmanage.db.get_value("Company", {"name": frm.doc.company}, "default_payroll_payable_account", (r) => {
	...
	currency: function (frm) {
		...
		if (!frm.doc.company) {
			company_currency = dontmanageerp.get_currency(dontmanage.defaults.get_default("Company"));
		} else {
			company_currency = dontmanageerp.get_currency(frm.doc.company);
		}
		if (frm.doc.currency) {
...
const submit_salary_slip = function (frm) {
	dontmanage.confirm(__('This will submit Salary Slips and create accrual Journal Entry. Do you want to proceed?'),
		function () {
			dontmanage.call({
...
let make_bank_entry = function (frm) {
	...
	if (doc.payment_account) {
		...
		dontmanage.msgprint(__("Payment Account is mandatory"));
...
let render_employee_attendance = function (frm, data) {
	frm.fields_dict.attendance_detail_html.html(
		dontmanage.render_template('employees_to_mark_attendance', {
			data: data
		})
	);
};
