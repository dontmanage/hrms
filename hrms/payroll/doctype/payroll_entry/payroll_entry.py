...
class PayrollEntry(Document):
	def onload(self):
		...
		entries = dontmanage.db.count("Salary Slip", {"payroll_entry": self.name, "docstatus": 1}, ["name"])
	...
	def before_submit(self):
		...
		if self.validate_attendance:
			if self.validate_employee_attendance():
				dontmanage.throw(_("Cannot Submit, Employees left to mark attendance"))
	...
	def validate_employee_details(self):
		...
		if len(emp_with_sal_slip):
			dontmanage.throw(_("Salary Slip already exists for {0}").format(comma_and(emp_with_sal_slip)))

	def validate_payroll_payable_account(self):
		if dontmanage.db.get_value("Account", self.payroll_payable_account, "account_type"):
			dontmanage.throw(
				_(
					"Account type cannot be set for payroll payable account {0}, please remove and try again"
				).format(dontmanage.bold(get_link_to_form("Account", self.payroll_payable_account)))
			)
	...
	def fill_employee_details(self):
		...
		if not employees:
			error_msg = _(
				...
				dontmanage.bold(self.currency),
				dontmanage.bold(self.payroll_payable_account),
			)
			if self.branch:
				error_msg += "<br>" + _("Branch: {0}").format(dontmanage.bold(self.branch))
			if self.department:
				error_msg += "<br>" + _("Department: {0}").format(dontmanage.bold(self.department))
			if self.designation:
				error_msg += "<br>" + _("Designation: {0}").format(dontmanage.bold(self.designation))
			if self.start_date:
				error_msg += "<br>" + _("Start date: {0}").format(dontmanage.bold(self.start_date))
			if self.end_date:
				error_msg += "<br>" + _("End date: {0}").format(dontmanage.bold(self.end_date))
			dontmanage.throw(error_msg, title=_("No employees found"))
	...
	def check_mandatory(self):
		for fieldname in ["company", "start_date", "end_date"]:
			if not self.get(fieldname):
				dontmanage.throw(_("Please set {0}").format(self.meta.get_label(fieldname)))
	...
	def create_salary_slips(self):
		...
		if employees:
			...
			if len(employees) > 30 or dontmanage.flags.enqueue_payroll_entry:
	...
	def submit_salary_slips(self):
		...
		if len(salary_slips) > 30 or dontmanage.flags.enqueue_payroll_entry:
			self.db_set("status", "Queued")
			dontmanage.enqueue(
	...
	def email_salary_slip(self, submitted_ss):
		if dontmanage.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee"):
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			ss = dontmanage.qb.DocType("Salary Slip")
			ssd = dontmanage.qb.DocType("Salary Detail")
			salary_components = (
				dontmanage.qb.from_(ss)
				.join(ssd)
		...
		if salary_components:
			...
			for item in salary_components:
				...
				if component_type == "earnings":
					is_flexible_benefit, only_tax_impact = dontmanage.get_cached_value(
	...
	def get_payroll_cost_centers_for_employee(self, employee, salary_structure):
		if not self.employee_cost_centers.get(employee):
			ss_assignment_name = dontmanage.db.get_value(
			...
			if ss_assignment_name:
				...
				if not cost_centers:
					default_cost_center, department = dontmanage.get_cached_value(
						"Employee", employee, ["payroll_cost_center", "department"]
					)
					if not default_cost_center and department:
						default_cost_center = dontmanage.get_cached_value(
	...
	def make_accrual_jv_entry(self):
		self.check_permission("write")
		process_payroll_accounting_entry_based_on_employee = dontmanage.db.get_single_value(
		...
		jv_name = ""
		precision = dontmanage.get_precision("Journal Entry Account", "debit_in_account_currency")

		if earnings or deductions:
			journal_entry = dontmanage.new_doc("Journal Entry")
	...
	def make_payment_entry(self):
		...
		salary_slip_name_list = dontmanage.db.sql(
		...
		if salary_slip_name_list and len(salary_slip_name_list) > 0:
			...
			for salary_slip_name in salary_slip_name_list:
				salary_slip = dontmanage.get_doc("Salary Slip", salary_slip_name[0])
				...
				for sal_detail in salary_slip.deductions:
					statistical_component = dontmanage.db.get_value(
	...
	def update_salary_slip_status(self, jv_name=None):
		...
		for ss in ss_list:
			ss_obj = dontmanage.get_doc("Salary Slip", ss[0])
			dontmanage.db.set_value("Salary Slip", ss_obj.name, "journal_entry", jv_name)
	...
	def validate_employee_attendance(self):
		...
		for employee_detail in self.employees:
			employee_joining_date = dontmanage.db.get_value(
	...
	def get_count_employee_attendance(self, employee, start_date):
		marked_days = 0
		attendances = dontmanage.get_all(
			"Attendance",
			fields=["count(*)"],
...
def get_filter_condition(filters):
	...
	for f in ["company", "branch", "department", "designation"]:
		if filters.get(f):
			cond += " and t1." + f + " = " + dontmanage.db.escape(filters.get(f))
...
def get_start_end_dates(payroll_frequency, start_date=None, company=None):
	...
	return dontmanage._dict({"start_date": start_date, "end_date": end_date})
...
def get_month_details(year, month):
	ysd = dontmanage.db.get_value("Fiscal Year", year, "year_start_date")
	if ysd:
		import calendar
		import datetime
		...
		dontmanage.throw(_("Fiscal Year {0} not found").format(year))


def get_payroll_entry_bank_entries(payroll_entry_name):
	je = dontmanage.qb.DocType("Journal Entry")
	jea = dontmanage.qb.DocType("Journal Entry Account")

	journal_entries = (
		dontmanage.qb.from_(je)
		.from_(jea)
		.select(je.name)
		.where(
			(je.name == jea.parent)
...
def log_payroll_failure(process, payroll_entry, error):
	error_log = dontmanage.log_error(
		title=_("Salary Slip {0} failed for Payroll Entry {1}").format(process, payroll_entry.name)
	)
	message_log = dontmanage.message_log.pop() if dontmanage.message_log else str(error)
...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
	try:
		payroll_entry = dontmanage.get_doc("Payroll Entry", args.payroll_entry)
		...
		if salary_slips_exist_for:
			dontmanage.msgprint(
				...
				).format(dontmanage.bold(", ".join(emp for emp in salary_slips_exist_for))),
				title=_("Message"),
				indicator="orange",
			)
		...
		dontmanage.publish_realtime("completed_salary_slip_creation")
...
def submit_salary_slips_for_employees(payroll_entry, salary_slips, publish_progress=True):
	try:
		submitted = []
		unsubmitted = []
		dontmanage.flags.via_payroll_entry = True
		count = 0

		for entry in salary_slips:
			salary_slip = dontmanage.get_doc("Salary Slip", entry[0])
			...
			count += 1
			if publish_progress:
				dontmanage.publish_progress(count * 100 / len(salary_slips), title=_("Submitting Salary Slips..."))
		...
		dontmanage.db.commit()  # nosemgrep
		dontmanage.publish_realtime("completed_salary_slip_submission")

	dontmanage.flags.via_payroll_entry = False
...
def get_employee_list(filters: dontmanage._dict) -> list[str]:
...
def employee_query(doctype, txt, searchfield, start, page_len, filters):
	filters = dontmanage._dict(filters)
	conditions = []
	include_employees = []
	emp_cond = ""
	doctype = "Employee"
	...
	if not filters.payroll_frequency:
		dontmanage.throw(_("Select Payroll Frequency."))
