...
from dontmanage.utils import add_months

import dontmanageerp
from dontmanageerp.accounts.utils import get_fiscal_year, getdate, nowdate
...
class TestPayrollEntry(DontManageTestCase):
	def setUp(self):
		for dt in [
			"Salary Slip",
			"Salary Component",
		...
		dontmanage.db.set_value("Company", "_Test Company", "default_holiday_list", "_Test Holiday List")
		dontmanage.db.set_value("Payroll Settings", None, "email_salary_slip_to_employee", 0)

		# set default payable account
		default_account = dontmanage.db.get_value(
	...
	def test_payroll_entry(self):
		company = dontmanage.get_doc("Company", "_Test Company")
		employee = dontmanage.db.get_value("Employee", {"company": "_Test Company"})
	...
	def test_multi_currency_payroll_entry(self):
		...
		salary_slip = dontmanage.db.get_value("Salary Slip", {"payroll_entry": payroll_entry.name}, "name")
		salary_slip = dontmanage.get_doc("Salary Slip", salary_slip)

		payroll_entry.reload()
		payroll_je = salary_slip.journal_entry
		if payroll_je:
			payroll_je_doc = dontmanage.get_doc("Journal Entry", payroll_je)
			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_debit)
			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_credit)

		payment_entry = dontmanage.db.sql(
			"""
			select
	...
	def test_payroll_entry_with_employee_cost_center(self):
		if not dontmanage.db.exists("Department", "cc - _TC"):
			dontmanage.get_doc(
		...
		pe = make_payroll_entry(
			...
			currency=dontmanage.db.get_value("Company", "_Test Company", "default_currency"),
			department="cc - _TC",
			company="_Test Company",
			payment_account="Cash - _TC",
			cost_center="Main - _TC",
		)
		je = dontmanage.db.get_value("Salary Slip", {"payroll_entry": pe.name}, "journal_entry")
		je_entries = dontmanage.db.sql(
			"""
	...
	def test_loan(self):
		...
		if not dontmanage.db.exists("Branch", branch):
			dontmanage.get_doc({"doctype": "Branch", "branch": branch}).insert()
		...
		if not dontmanage.db.exists("Loan Type", "Car Loan"):
			create_loan_accounts()
			create_loan_type(
				"Car Loan",
				500000,
		...
		salary_slip = dontmanage.get_doc("Salary Slip", name)
	...
	def test_salary_slip_operation_queueing(self):
		...
		dontmanage.flags.enqueue_payroll_entry = True
		payroll_entry.submit()
		payroll_entry.reload()

		self.assertEqual(payroll_entry.status, "Queued")
		dontmanage.flags.enqueue_payroll_entry = False
	...
	def test_salary_slip_operation_failure(self):
		...
		component = dontmanage.get_doc("Salary Component", salary_structure.earnings[0].salary_component)
		component.accounts = []
		component.save()
		...
		dontmanage.db.set_value("Employee", employee, "status", "Inactive")
		payroll_entry.submit()
		payroll_entry.reload()
		self.assertEqual(payroll_entry.status, "Failed")
		self.assertIsNotNone(payroll_entry.error_message)

		dontmanage.db.set_value("Employee", employee, "status", "Active")
		payroll_entry.submit()
		...
		# set accounts
		for data in dontmanage.get_all("Salary Component", pluck="name"):
	...
	def test_payroll_entry_cancellation_against_cacnelled_gernal_entry(self):
		company_doc = dontmanage.get_doc("Company", "_Test Company")
		...
		jv_doc = dontmanage.get_doc("Journal Entry", jv.parent)
...
def get_payroll_entry(**args):
	...
	payroll_entry: PayrollEntry = dontmanage.new_doc("Payroll Entry")
	payroll_entry.company = args.company or dontmanageerp.get_default_company()
...
def get_payment_account():
	return dontmanage.get_value(
		"Account",
		{"account_type": "Cash", "company": dontmanageerp.get_default_company(), "is_group": 0},
		"name",
	)


def make_holiday(holiday_list_name):
	if not dontmanage.db.exists("Holiday List", holiday_list_name):
