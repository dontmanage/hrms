...
from dontmanage.utils import comma_and, date_diff, formatdate, getdate
...
class AdditionalSalary(Document):
	...
	def validate(self):
		...
		if self.amount < 0:
			dontmanage.throw(_("Amount should not be less than zero"))
	...
	def validate_recurring_additional_salary_overlap(self):
		if self.is_recurring:
			AdditionalSalary = dontmanage.qb.DocType("Additional Salary")

			additional_salaries = (
				dontmanage.qb.from_(AdditionalSalary)
	...
	def validate_dates(self):
		...
		if getdate(self.from_date) > getdate(self.to_date):
			dontmanage.throw(_("From Date can not be greater than To Date."))

		if date_of_joining:
			if self.payroll_date and getdate(self.payroll_date) < getdate(date_of_joining):
				dontmanage.throw(_("Payroll date can not be less than employee's joining date."))
		...
		if relieving_date:
			if self.to_date and getdate(self.to_date) > getdate(relieving_date):
				dontmanage.throw(_("To date can not be greater than employee's relieving date."))
			if self.payroll_date and getdate(self.payroll_date) > getdate(relieving_date):
				dontmanage.throw(_("Payroll date can not be greater than employee's relieving date."))

	def validate_employee_referral(self):
		if self.ref_doctype == "Employee Referral":
			referral_details = dontmanage.db.get_value(
				"Employee Referral",
				self.ref_docname,
			...
			if self.type == "Deduction":
				dontmanage.throw(_("Earning Salary Component is required for Employee Referral Bonus."))
			...
			if referral_details.status != "Accepted":
				dontmanage.throw(
					...
					).format(dontmanage.bold("Accepted"))
				)

	def update_return_amount_in_employee_advance(self):
		if self.ref_doctype == "Employee Advance" and self.ref_docname:
			return_amount = dontmanage.db.get_value("Employee Advance", self.ref_docname, "return_amount")
			...
			dontmanage.db.set_value("Employee Advance", self.ref_docname, "return_amount", return_amount)
			advance = dontmanage.get_doc("Employee Advance", self.ref_docname)
			advance.set_status(update=True)

	def update_employee_referral(self, cancel=False):
		if self.ref_doctype == "Employee Referral":
			status = "Unpaid" if cancel else "Paid"
			dontmanage.db.set_value("Employee Referral", self.ref_docname, "referral_payment_status", status)
...
def get_additional_salaries(employee, start_date, end_date, component_type):
	...
	additional_sal = dontmanage.qb.DocType("Additional Salary")
	...
	additional_salary_list = (
		dontmanage.qb.from_(additional_sal)
		.select(
			additional_sal.name,
			component_field,
			additional_sal.type,
		...
		if d.overwrite:
			if d.component in components_to_overwrite:
				dontmanage.throw(
					...
					).format(dontmanage.bold(d.component), start_date, end_date),
					title=_("Error"),
				)
