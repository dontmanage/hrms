...
from dontmanage.utils import add_years, date_diff, get_first_day, nowdate
...
class TestSalaryStructure(DontManageTestCase):
	def setUp(self):
		...
		dontmanage.db.set_value(
			"Company",
			dontmanageerp.get_default_company(),
	...
	def make_holiday_list(self):
		if not dontmanage.db.get_value("Holiday List", "Salary Structure Test Holiday List"):
	...
	def test_amount_totals(self):
		...
		sal_slip = dontmanage.get_value("Salary Slip", {"employee_name": "test_employee_2@salary.com"})
		if not sal_slip:
	...
	def test_salary_structures_assignment(self):
		company_currency = dontmanageerp.get_default_currency()
		...
		salary_structure_assignment = dontmanage.get_doc(
	...
	def test_employee_grade_defaults(self):
		...
		structure, base = dontmanage.db.get_value(
...
def make_salary_structure(
	...
	if (
		employee
		and not dontmanage.db.get_value("Salary Structure Assignment", filters)
...
def create_salary_structure_assignment(
	...
	if not allow_duplicate and dontmanage.db.exists(
		"Salary Structure Assignment", {"employee": employee}
	):
		dontmanage.db.sql("""delete from `tabSalary Structure Assignment` where employee=%s""", (employee))

	if not payroll_period:
		payroll_period = create_payroll_period()

	income_tax_slab = dontmanage.db.get_value("Income Tax Slab", {"currency": currency})

	if not income_tax_slab:
		income_tax_slab = create_tax_slab(payroll_period, allow_tax_exemption=True, currency=currency)

	salary_structure_assignment = dontmanage.new_doc("Salary Structure Assignment")
	...
	if not from_date:
		...
		joining_date = dontmanage.get_cached_value("Employee", employee, "date_of_joining")
	...
	salary_structure_assignment.company = company or dontmanageerp.get_default_company()
...
def get_payable_account(company=None):
	...
	return dontmanage.db.get_value("Company", company, "default_payroll_payable_account")


def create_employee_grade(grade, default_structure=None):
	if not dontmanage.db.exists("Employee Grade", grade):
		dontmanage.get_doc(
			{
