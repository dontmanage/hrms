...
{% include "dontmanageerp/public/js/controllers/accounts.js" %}
...
dontmanage.ui.form.on('Salary Structure', {
	onload: function(frm) {
		...
		let help_button = $(`<a class = 'control-label'>
			...
			let message_html = dontmanage.render_template("condition_and_formula_help")
	...
	refresh: function(frm) {
		...
		if(frm.doc.docstatus==1) {
			frm.add_custom_button(__("Assign Salary Structure"), function() {
				var doc = dontmanage.model.get_new_doc('Salary Structure Assignment');
				doc.salary_structure = frm.doc.name;
				doc.company = frm.doc.company;
				dontmanage.set_route('Form', 'Salary Structure Assignment', doc.name);
			});
	...
	assign_to_employees:function (frm) {
		...
		d.fields_dict.grade.df.onchange = function() {
			...
			if (grade) {
				dontmanage.db.get_value('Employee Grade', grade, 'default_base_pay')
					.then(({ message }) => {
...
var validate_date = function(frm, cdt, cdn) {
	...
	if(doc.to_date && doc.from_date) {
		var from_date = dontmanage.datetime.str_to_obj(doc.from_date);
		var to_date = dontmanage.datetime.str_to_obj(doc.to_date);

		if(to_date < from_date) {
			dontmanage.model.set_value(cdt, cdn, "to_date", "");
			dontmanage.throw(__("From Date cannot be greater than To Date"));
		}
	}
}
...
dontmanage.ui.form.on('Salary Detail', {
	amount: function(frm) {
		calculate_totals(frm.doc);
	},
			...
			dontmanage.call({
				...
				callback: function(data) {
					if(data.message){
						...
						dontmanage.model.set_value(cdt, cdn, 'condition', result.condition);
						dontmanage.model.set_value(cdt, cdn, 'amount_based_on_formula', result.amount_based_on_formula);
						if(result.amount_based_on_formula == 1){
							dontmanage.model.set_value(cdt, cdn, 'formula', result.formula);
						}
						else{
							dontmanage.model.set_value(cdt, cdn, 'amount', result.amount);
						}
						dontmanage.model.set_value(cdt, cdn, 'statistical_component', result.statistical_component);
						dontmanage.model.set_value(cdt, cdn, 'depends_on_payment_days', result.depends_on_payment_days);
						dontmanage.model.set_value(cdt, cdn, 'do_not_include_in_total', result.do_not_include_in_total);
						dontmanage.model.set_value(cdt, cdn, 'variable_based_on_taxable_salary', result.variable_based_on_taxable_salary);
						dontmanage.model.set_value(cdt, cdn, 'is_tax_applicable', result.is_tax_applicable);
						dontmanage.model.set_value(cdt, cdn, 'is_flexible_benefit', result.is_flexible_benefit);
	...
	amount_based_on_formula: function(frm, cdt, cdn) {
		...
		if(child.amount_based_on_formula == 1){
			dontmanage.model.set_value(cdt, cdn, 'amount', null);
		}
		else{
			dontmanage.model.set_value(cdt, cdn, 'formula', null);
		}
	}
})
