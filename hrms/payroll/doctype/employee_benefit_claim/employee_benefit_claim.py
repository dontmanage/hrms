...
class EmployeeBenefitClaim(Document):
	def validate(self):
		...
		if not max_benefits or max_benefits <= 0:
			dontmanage.throw(_("Employee {0} has no maximum benefit amount").format(self.employee))
		payroll_period = get_payroll_period(
			self.claim_date, self.claim_date, dontmanage.db.get_value("Employee", self.employee, "company")
		)
		if not payroll_period:
			dontmanage.throw(
				_("{0} is not in a valid Payroll Period").format(
					dontmanage.format(self.claim_date, dict(fieldtype="Date"))
				)
			)
...
def get_benefit_pro_rata_ratio_amount(employee, on_date, sal_struct):
	...
	for sal_struct_row in sal_struct.get("earnings"):
		try:
			...
			dontmanage.throw(_("Unable to find Salary Component {0}").format(sal_struct_row.salary_component))
...
def get_total_benefit_dispensed(employee, sal_struct, sal_slip_start_date, payroll_period):
	...
	if application:
		application_obj = dontmanage.get_doc("Employee Benefit Application", application)
		pro_rata_amount = (
	...
	if remaining_benefit > 0:
		...
		for d in sal_struct.get("earnings"):
			if d.is_flexible_benefit == 1:
				salary_component = dontmanage.get_doc("Salary Component", d.salary_component)
