...
from dontmanage.utils import add_months, cint, date_diff, flt, formatdate, getdate, month_diff
...
class PayrollPeriod(Document):
	...
	def validate_dates(self):
		if getdate(self.start_date) > getdate(self.end_date):
			dontmanage.throw(_("End date can not be less than start date"))
...
def get_payroll_period_days(start_date, end_date, employee, company=None):
	...
	payroll_period = dontmanage.db.sql(
		"""
	...
	if len(payroll_period) > 0:
		...
		if not cint(
			dontmanage.db.get_value("Payroll Settings", None, "include_holidays_in_total_working_days")
		):
...
def get_payroll_period(from_date, to_date, company):
	PayrollPeriod = dontmanage.qb.DocType("Payroll Period")

	payroll_period = (
		dontmanage.qb.from_(PayrollPeriod)
