...
from dontmanage.utils import add_days, add_months, floor, flt, get_datetime, get_first_day, getdate
...
class TestGratuity(DontManageTestCase):
	def setUp(self):
		dontmanage.db.delete("Gratuity")
		dontmanage.db.delete("Salary Slip")
		dontmanage.db.delete("Additional Salary", {"ref_doctype": "Gratuity"})
	...
	def test_gratuity_based_on_current_slab_via_additional_salary(self):
		...
		# amount calculation
		component_amount = dontmanage.get_all(
			"Salary Detail",
			filters={
				"docstatus": 1,
				"parent": sal_slip,
		...
		self.assertTrue(dontmanage.db.exists("Additional Salary", {"ref_docname": gratuity.name}))
...
def get_gratuity_rule(name):
	rule = dontmanage.db.exists("Gratuity Rule", name)
	if not rule:
		setup()
	rule = dontmanage.get_doc("Gratuity Rule", name)
...
def create_gratuity(**args):
	...
	gratuity = dontmanage.new_doc("Gratuity")
...
def set_mode_of_payment_account():
	if not dontmanage.db.exists("Account", "Payment Account - _TC"):
		mode_of_payment = create_account()

	mode_of_payment = dontmanage.get_doc("Mode of Payment", "Cash")
...
def create_salary_slip(employee):
	if not dontmanage.db.exists("Salary Slip", {"employee": employee}):
