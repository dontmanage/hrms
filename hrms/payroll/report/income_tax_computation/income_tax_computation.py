...
from dontmanage.utils import add_days, flt, getdate, rounded
...
class IncomeTaxComputationReport(object):
	def __init__(self, filters=None):
		...
		self.columns = []
		self.data = []
		self.employees = dontmanage._dict()
		self.payroll_period_start_date = None
		self.payroll_period_end_date = None
		if self.filters.payroll_period:
			self.payroll_period_start_date, self.payroll_period_end_date = dontmanage.db.get_value(
	...
	def get_employee_details(self):
		...
		employees = dontmanage.get_all("Employee", filters=filters, or_filters=or_filters, fields=fields)
		...
		if not self.employees:
			dontmanage.throw(_("No employees found with selected filters and active salary structure"))
	...
	def get_ss_assignments(self, employees):
		ss_assignments = dontmanage.get_all(
	...
	def get_future_salary_slips(self):
		self.future_salary_slips = dontmanage._dict()
		for employee in list(self.employees.keys()):
			...
			while getdate(ss_start_date) < getdate(self.payroll_period_end_date) and (
				...
				ss = dontmanage.new_doc("Salary Slip")
	...
	def get_last_salary_slip(self, employee):
		last_salary_slip = dontmanage.db.get_value(
			"Salary Slip",
			{
				"employee": employee,
				"docstatus": 1,
	...
	def get_tax_exempted_earnings_and_deductions(self):
		...
		ss = dontmanage.qb.DocType("Salary Slip")
		ss_comps = dontmanage.qb.DocType("Salary Detail")
		...
		existing_ss_exemptions = dontmanage._dict()
		for d in records:
	...
	def get_employee_tax_exemptions(self):
		# add columns
		exemption_categories = dontmanage.get_all("Employee Tax Exemption Category", {"is_active": 1})
	...
	def get_tax_exemptions(self, source):
		...
		par = dontmanage.qb.DocType(source)
		child = dontmanage.qb.DocType(child_doctype)

		records = (
			dontmanage.qb.from_(par)
			.inner_join(child)
	...
	def get_hra(self):
		if not dontmanage.get_meta("Employee Tax Exemption Declaration").has_field("monthly_house_rent"):
			return
	...
	def get_applicable_tax(self):
		...
		is_tax_rounded = dontmanage.db.get_value(
			"Salary Component",
		...
		for emp, emp_details in self.employees.items():
			...
			if tax_slab:
				tax_slab = dontmanage.get_cached_doc("Income Tax Slab", tax_slab)
				employee_dict = dontmanage.get_doc("Employee", emp).as_dict()
	...
	def get_total_deducted_tax(self):
		...
		ss = dontmanage.qb.DocType("Salary Slip")
		ss_ded = dontmanage.qb.DocType("Salary Detail")
