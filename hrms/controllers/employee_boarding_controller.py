...
from dontmanage.utils import add_days, flt, unique
...
class EmployeeBoardingController(Document):
	...
	def create_task_and_notify_user(self):
		...
		for activity in self.activities:
			...
			if activity.role:
				user_list = dontmanage.db.sql_list(
					"""
					SELECT
		...
		if self.doctype == "Employee Separation":
			...
			if self.employee:
				...
				if not self.holiday_list:
					dontmanage.throw(_("Please set the Holiday List."), dontmanage.MandatoryError)
				else:
	...
	def on_cancel(self):
		# delete task project
		project = self.project
		for task in dontmanage.get_all("Task", filters={"project": project}):
			dontmanage.delete_doc("Task", task.name, force=1)
		dontmanage.delete_doc("Project", project, force=1)
		self.db_set("project", "")
		for activity in self.activities:
...
def update_employee_boarding_status(project, event=None):
	employee_onboarding = dontmanage.db.exists("Employee Onboarding", {"project": project.name})
	employee_separation = dontmanage.db.exists("Employee Separation", {"project": project.name})
	...
	if employee_onboarding:
		dontmanage.db.set_value("Employee Onboarding", employee_onboarding, "boarding_status", status)
	elif employee_separation:
		dontmanage.db.set_value("Employee Separation", employee_separation, "boarding_status", status)


def update_task(task, event=None):
	if task.project and not task.flags.from_project:
		update_employee_boarding_status(dontmanage.get_cached_doc("Project", task.project))
