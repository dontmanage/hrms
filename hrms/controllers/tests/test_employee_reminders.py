...
class TestEmployeeReminders(unittest.TestCase):
	@classmethod
	def setUpClass(cls):
		from dontmanageerp.setup.doctype.holiday_list.test_holiday_list import make_holiday_list
		...
		test_employee = dontmanage.get_doc(
		...
		test_employee_2 = dontmanage.get_doc("Employee", test_employee_2)
	...
	def setUp(self):
		...
		dontmanage.db.sql("delete from `tabEmail Queue Recipient`")
	...
	def test_birthday_reminders(self):
		employee = dontmanage.get_doc(
			"Employee", dontmanage.db.sql_list("select name from tabEmployee limit 1")[0]
		)
		employee.date_of_birth = "1992" + dontmanage.utils.nowdate()[4:]
		...
		hr_settings = dontmanage.get_doc("HR Settings", "HR Settings")
	...
	def test_work_anniversary_reminders(self):
		...
		emp = make_employee(
			...
			date_of_joining=dontmanage.utils.add_years(getdate(), -2),
		)
	...
	def test_advance_holiday_reminders_monthly(self):
		...
		recipients = dontmanage.db.get_all("Email Queue Recipient", pluck="recipient")
