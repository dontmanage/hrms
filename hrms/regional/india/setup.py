...
def add_custom_roles_for_reports():
	for report_name in (
		...
		if not dontmanage.db.get_value("Custom Role", dict(report=report_name)):
			dontmanage.get_doc(
				dict(
					doctype="Custom Role",
					report=report_name,
...
def create_gratuity_rule_for_india():
	if not dontmanage.db.exists("DocType", "Gratuity Rule"):
		return

	if dontmanage.db.exists("Gratuity Rule", "Indian Standard Gratuity Rule"):
		return

	rule = dontmanage.new_doc("Gratuity Rule")
	rule.update(
		{
