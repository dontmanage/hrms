...
from dontmanage.utils import add_days, date_diff, flt, get_link_to_form, month_diff
...
def calculate_annual_eligible_hra_exemption(doc):
	basic_component, hra_component = dontmanage.db.get_value(
	...
	if hra_component and basic_component:
		...
		for idx, assignment in enumerate(assignments):
			if has_hra_component(assignment.salary_structure, hra_component):
				...
				frequency = dontmanage.get_value(
...
def get_end_date_for_assignment(assignment_dates, idx, payroll_period):
	...
	if not end_date:
		end_date = dontmanage.db.get_value("Payroll Period", payroll_period, "end_date")
...
def validate_house_rent_dates(doc):
	if not doc.rented_to_date or not doc.rented_from_date:
		dontmanage.throw(_("House rented dates required for exemption calculation"))

	if date_diff(doc.rented_to_date, doc.rented_from_date) < 14:
		dontmanage.throw(_("House rented dates should be atleast 15 days apart"))

	proofs = dontmanage.db.sql(
		"""
		select name
	...
	if proofs:
		dontmanage.throw(_("House rent paid days overlapping with {0}").format(proofs[0][0]))
